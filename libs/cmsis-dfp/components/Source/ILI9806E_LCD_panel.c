...
static LCD_Page_cmd page1_cfg[] = {
    {0x08, 0x00}, {0x20, 0x00}, {0x21, 0x01}, {0x30, 0x02}, {0x31, 0x00}, {0x40, 0x00},
    {0x41, 0x33}, {0x42, 0x02}, {0x43, 0x89}, {0x44, 0x89}, {0x46, 0x34}, {0x50, 0xA8},
    {0x51, 0xA8}, {0x52, 0x00}, {0x53, 0x78}, {0x54, 0x00}, {0x55, 0x78}, {0x60, 0x07},
    {0x61, 0x04}, {0x62, 0x08}, {0x63, 0x04}, {0xA0, 0x00}, {0xA1, 0x0B}, {0xA2, 0x13},
    {0xA3, 0x0D}, {0xA4, 0x07}, {0xA5, 0x0B}, {0xA6, 0x07}, {0xA7, 0x06}, {0xA8, 0x07},
    {0xA9, 0x0A}, {0xAA, 0x12}, {0xAB, 0x0D}, {0xAC, 0x11}, {0xAD, 0x0F}, {0xAE, 0x0E},
    {0xAF, 0x0B}, {0xC0, 0x00}, {0xC1, 0x0B}, {0xC2, 0x13}, {0xC3, 0x0D}, {0xC4, 0x06},
    {0xC5, 0x0B}, {0xC6, 0x07}, {0xC7, 0x06}, {0xC8, 0x07}, {0xC9, 0x0A}, {0xCA, 0x12},
    {0xCB, 0x0D}, {0xCC, 0x11}, {0xCD, 0x0F}, {0xCE, 0x0E}, {0xCF, 0x0B}};

/*Commands for Page 6*/
static LCD_Page_cmd page6_cfg[] = {
    {0x00, 0x21}, {0x01, 0x09}, {0x02, 0x00}, {0x03, 0x00}, {0x04, 0x01}, {0x05, 0x01},
    {0x06, 0x98}, {0x07, 0x05}, {0x08, 0x02}, {0x09, 0x00}, {0x0A, 0x00}, {0x0B, 0x00},
    {0x0C, 0x01}, {0x0D, 0x01}, {0x0E, 0x00}, {0x0F, 0x00}, {0x10, 0xE0}, {0x11, 0xE0},
    {0x12, 0x00}, {0x13, 0x00}, {0x14, 0x00}, {0x15, 0x43}, {0x16, 0x08}, {0x17, 0x00},
    {0x18, 0x00}, {0x19, 0x00}, {0x1A, 0x00}, {0x1B, 0x00}, {0x1C, 0x00}, {0x1D, 0x00},
    {0x20, 0x01}, {0x21, 0x23}, {0x22, 0x45}, {0x23, 0x67}, {0x24, 0x01}, {0x25, 0x23},
    {0x26, 0x45}, {0x27, 0x67}, {0x30, 0x01}, {0x31, 0x11}, {0x32, 0x00}, {0x33, 0x22},
    {0x34, 0x22}, {0x35, 0xCB}, {0x36, 0xDA}, {0x37, 0xAD}, {0x38, 0xBC}, {0x39, 0x66},
    {0x3A, 0x77}, {0x3B, 0x22}, {0x3C, 0x22}, {0x3D, 0x22}, {0x3E, 0x22}, {0x3F, 0x22},
    {0x40, 0x22}};

/*Commands for Page 7*/
static LCD_Page_cmd page7_cfg[] = {{0x18, 0x1D}, {0x02, 0x77}, {0xE1, 0x79}};

/*Commands for Page 0*/
static LCD_Page_cmd page0_cfg[] = {{0x36, 0x01}, {0x3A, 0x70}};
...
/*Commands for Page 1*/
static LCD_Page_cmd page1_cfg[] = {
    {0x08, 0x10}, {0x20, 0x00}, {0x21, 0x01}, {0x30, 0x02}, {0x31, 0x00}, {0x40, 0x14},
    {0x41, 0x22}, {0x42, 0x02}, {0x43, 0x84}, {0x44, 0x8A}, {0x50, 0x78}, {0x51, 0x78},
    {0x52, 0x00}, {0x53, 0x2B}, {0x54, 0x00}, {0x55, 0x2B}, {0x60, 0x07}, {0x61, 0x06},
    {0x62, 0x06}, {0x63, 0x04}, {0xA0, 0x00}, {0xA1, 0x0B}, {0xA2, 0x19}, {0xA3, 0x10},
    {0xA4, 0x06}, {0xA5, 0x0F}, {0xA6, 0x09}, {0xA7, 0x06}, {0xA8, 0x0C}, {0xA9, 0x0E},
    {0xAA, 0x16}, {0xAB, 0x0D}, {0xAC, 0x15}, {0xAD, 0x0F}, {0xAE, 0x11}, {0xAF, 0x00},
    {0xC0, 0x00}, {0xC1, 0x24}, {0xC2, 0x29}, {0xC3, 0x0C}, {0xC4, 0x07}, {0xC5, 0x03},
    {0xC6, 0x03}, {0xC7, 0x03}, {0xC8, 0x03}, {0xC9, 0x09}, {0xCA, 0x0D}, {0xCB, 0x01},
    {0xCC, 0x06}, {0xCD, 0x1B}, {0xCE, 0x08}, {0xCF, 0x00}};

/*Commands for Page 6*/
static LCD_Page_cmd page6_cfg[] = {
    {0x00, 0x20}, {0x01, 0x04}, {0x02, 0x00}, {0x03, 0x00}, {0x04, 0x01}, {0x05, 0x01},
    {0x06, 0x88}, {0x07, 0x04}, {0x08, 0x01}, {0x09, 0x90}, {0x0A, 0x03}, {0x0B, 0x01},
    {0x0C, 0x01}, {0x0D, 0x01}, {0x0E, 0x00}, {0x0F, 0x00}, {0x10, 0x55}, {0x11, 0x53},
    {0x12, 0x01}, {0x13, 0x0D}, {0x14, 0x0D}, {0x15, 0x43}, {0x16, 0x0B}, {0x17, 0x00},
    {0x18, 0x00}, {0x19, 0x00}, {0x1A, 0x00}, {0x1B, 0x00}, {0x1C, 0x00}, {0x1D, 0x00},
    {0x20, 0x01}, {0x21, 0x23}, {0x22, 0x45}, {0x23, 0x67}, {0x24, 0x01}, {0x25, 0x23},
    {0x26, 0x45}, {0x27, 0x67}, {0x30, 0x02}, {0x31, 0x22}, {0x32, 0x11}, {0x33, 0xAA},
    {0x34, 0xBB}, {0x35, 0x66}, {0x36, 0x00}, {0x37, 0x22}, {0x38, 0x22}, {0x39, 0x22},
    {0x3A, 0x22}, {0x3B, 0x22}, {0x3C, 0x22}, {0x3D, 0x22}, {0x3E, 0x22}, {0x3F, 0x22},
    {0x40, 0x22}};

/*Commands for Page 5*/
static LCD_Page_cmd page5_cfg[] = {{0x09, 0xFC}, {0x07, 0xBC}};

/*Commands for Page 0*/
static LCD_Page_cmd page0_cfg[] = {{0x3A, 0x70}};
...
/*Commands for Page 1*/
static LCD_Page_cmd page1_cfg[] = {
    {0x08, 0x10}, {0x20, 0x00}, {0x21, 0x01}, {0x30, 0x01}, {0x31, 0x00}, {0x40, 0x16},
    {0x41, 0x33}, {0x42, 0x03}, {0x43, 0x09}, {0x44, 0x06}, {0x50, 0x88}, {0x51, 0x88},
    {0x52, 0x00}, {0x53, 0x49}, {0x55, 0x49}, {0x60, 0x07}, {0x61, 0x00}, {0x62, 0x07},
    {0x63, 0x00}, {0xA0, 0x00}, {0xA1, 0x09}, {0xA2, 0x11}, {0xA3, 0x0B}, {0xA4, 0x05},
    {0xA5, 0x08}, {0xA6, 0x06}, {0xA7, 0x04}, {0xA8, 0x09}, {0xA9, 0x0C}, {0xAA, 0x15},
    {0xAB, 0x08}, {0xAC, 0x0F}, {0xAD, 0x12}, {0xAE, 0x09}, {0xAF, 0x00}, {0xC0, 0x00},
    {0xC1, 0x09}, {0xC2, 0x10}, {0xC3, 0x0C}, {0xC4, 0x05}, {0xC5, 0x08}, {0xC6, 0x06},
    {0xC7, 0x04}, {0xC8, 0x08}, {0xC9, 0x0C}, {0xCA, 0x14}, {0xCB, 0x08}, {0xCC, 0x0F},
    {0xCD, 0x11}, {0xCE, 0x09}, {0xCF, 0x00}};

/*Commands for Page 6*/
static LCD_Page_cmd page6_cfg[] = {
    {0x00, 0x20}, {0x01, 0x0A}, {0x02, 0x00}, {0x03, 0x00}, {0x04, 0x01}, {0x05, 0x01},
    {0x06, 0x98}, {0x07, 0x06}, {0x08, 0x01}, {0x09, 0x80}, {0x0A, 0x00}, {0x0B, 0x00},
    {0x0C, 0x01}, {0x0D, 0x01}, {0x0E, 0x05}, {0x0F, 0x00}, {0x10, 0xF0}, {0x11, 0xF4},
    {0x12, 0x01}, {0x13, 0x00}, {0x14, 0x00}, {0x15, 0xC0}, {0x16, 0x08}, {0x17, 0x00},
    {0x18, 0x00}, {0x19, 0x00}, {0x1A, 0x00}, {0x1B, 0x00}, {0x1C, 0x00}, {0x1D, 0x00},
    {0x20, 0x01}, {0x21, 0x23}, {0x22, 0x45}, {0x23, 0x67}, {0x24, 0x01}, {0x25, 0x23},
    {0x26, 0x45}, {0x27, 0x67}, {0x30, 0x11}, {0x31, 0x11}, {0x32, 0x00}, {0x33, 0xEE},
    {0x34, 0xFF}, {0x35, 0xBB}, {0x36, 0xAA}, {0x37, 0xDD}, {0x38, 0xCC}, {0x39, 0x66},
    {0x3A, 0x77}, {0x3B, 0x22}, {0x3C, 0x22}, {0x3D, 0x22}, {0x3E, 0x22}, {0x3F, 0x22},
    {0x40, 0x22}};

/*Commands for Page 5*/
static LCD_Page_cmd page5_cfg[] = {{0x17, 0x22}, {0x02, 0x77}, {0x26, 0xB2}};
