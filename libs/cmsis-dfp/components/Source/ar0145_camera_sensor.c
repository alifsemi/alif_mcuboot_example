...
static ARM_DRIVER_GPIO *GPIO_Driver_CAM_RST =
    &ARM_Driver_GPIO_(RTE_AR0145_CAMERA_SENSOR_RESET_GPIO_PORT);
...
static ARM_DRIVER_GPIO *GPIO_Driver_CAM_PWR =
    &ARM_Driver_GPIO_(RTE_AR0145_CAMERA_SENSOR_POWER_GPIO_PORT);
...
/**
  ...
  */
static int32_t AR0145_Bulk_Write_Reg(const AR0145_REG AR0145_reg[], uint32_t total_num,
                                     uint32_t reg_size)
{
    uint32_t i   = 0;
    int32_t  ret = 0;
    ...
    for (i = 0; i < total_num; i++) {
        ...
        ret = AR0145_WRITE_REG(AR0145_reg[i].reg_addr, AR0145_reg[i].reg_value, reg_size);
...
static int32_t AR0145_Camera_Hard_Reseten(void)
{
    ...
    ret = GPIO_Driver_CAM_RST->SetDirection(RTE_AR0145_CAMERA_SENSOR_RESET_PIN_NO,
    ...
    ret = GPIO_Driver_CAM_PWR->SetDirection(RTE_AR0145_CAMERA_SENSOR_POWER_PIN_NO,
                                            GPIO_PIN_DIRECTION_OUTPUT);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = GPIO_Driver_CAM_RST->SetValue(RTE_AR0145_CAMERA_SENSOR_RESET_PIN_NO,
    ...
    ret = GPIO_Driver_CAM_PWR->SetValue(RTE_AR0145_CAMERA_SENSOR_POWER_PIN_NO,
...
int32_t AR0145_Sensor_Configuration(void)
{
    ...
    ret = AR0145_WRITE_REG(AR0145_COARSE_INTEGRATION_TIME_REGISTER,
                           AR0145_CAMERA_SENSOR_FRAME_HEIGHT + 31,
                           2);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }
...
static int32_t AR0145_Camera_Gain(const uint32_t gain)
{
    ...
    if (max_integration_time == 0) {
         ...
         */
        int32_t ret =
            AR0145_READ_REG(AR0145_COARSE_INTEGRATION_TIME_REGISTER, &current_integration_time, 2);
        if (ret != 0) {
            return ret;
        }
    ...
    if (gain != 0) {
        ...
        if (new_integration_time != current_integration_time) {
            int32_t ret =
                AR0145_WRITE_REG(AR0145_COARSE_INTEGRATION_TIME_REGISTER, new_integration_time, 2);
            if (ret != 0) {
                return ret;
            }
        ...
        int32_t ret =
            AR0145_WRITE_REG(AR0145_GLOBAL_GAIN_REGISTER, (digital_gain << 7) | (s << 4) | t, 2);
        if (ret != 0) {
            return ret;
        }
    } else {
    ...
    // And adjust for any reduction of integration time (in which case gain should be 0x10000
    // initially...)
    if (current_integration_time != max_integration_time) {
        resulting_gain =
            ((resulting_gain * current_integration_time) / max_integration_time + 0.5f);
    }
...
static int32_t AR0145_Camera_AE(const uint32_t enable)
{
    ...
    if (enable) {
        ...
        ret = AR0145_WRITE_REG(AR0145_AE_STATS_CONTROL_REGISTER,
                               AR0145_EMBEDDED_STATISTICS_ENABLE,
                               2);
        if (ret != ARM_DRIVER_OK) {
            return ret;
        }
...
\ref CSI_INFO
*/
static CSI_INFO AR0145_csi_info            = {.frequency = RTE_AR0145_CAMERA_SENSOR_CSI_FREQ,
                                              ...
                                              .cpi_cfg.override =
                                                  RTE_AR0145_CAMERA_SENSOR_OVERRIDE_CPI_COLOR_MODE,
                                              .cpi_cfg.cpi_color_mode =
                                                  RTE_AR0145_CAMERA_SENSOR_CPI_COLOR_MODE};
