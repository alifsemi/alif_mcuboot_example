...
static ARM_DRIVER_GPIO *GPIO_Driver_CAM_RST =
    &ARM_Driver_GPIO_(RTE_ARX3A0_CAMERA_SENSOR_RESET_GPIO_PORT);
...
static ARM_DRIVER_GPIO *GPIO_Driver_CAM_PWR =
    &ARM_Driver_GPIO_(RTE_ARX3A0_CAMERA_SENSOR_POWER_GPIO_PORT);
...
/**
  ...
  */
static int32_t ARX3A0_Bulk_Write_Reg(const ARX3A0_REG arx3A0_reg[], uint32_t total_num,
                                     uint32_t reg_size)
{
    uint32_t i   = 0;
    int32_t  ret = 0;
    ...
    for (i = 0; i < total_num; i++) {
        ret = ARX3A0_WRITE_REG(arx3A0_reg[i].reg_addr, arx3A0_reg[i].reg_value, reg_size);
...
static int32_t ARX3A0_Camera_Hard_Reseten(void)
{
    ...
    ret = GPIO_Driver_CAM_RST->SetDirection(RTE_ARX3A0_CAMERA_SENSOR_RESET_PIN_NO,
    ...
    ret = GPIO_Driver_CAM_PWR->SetDirection(RTE_ARX3A0_CAMERA_SENSOR_POWER_PIN_NO,
                                            GPIO_PIN_DIRECTION_OUTPUT);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = GPIO_Driver_CAM_RST->SetValue(RTE_ARX3A0_CAMERA_SENSOR_RESET_PIN_NO,
    ...
    ret = GPIO_Driver_CAM_PWR->SetValue(RTE_ARX3A0_CAMERA_SENSOR_POWER_PIN_NO,
...
static int32_t ARX3A0_Camera_Gain(const uint32_t gain)
{
    ...
    if (max_integration_time == 0) {
        ...
        /* Slight hackery - if this API is in use, assume the caller does not want the data pedestal
         * - disable it */
    ...
    if (gain != 0) {
        ...
        if (new_integration_time != current_integration_time) {
            int32_t ret =
                ARX3A0_WRITE_REG(ARX3A0_COARSE_INTEGRATION_TIME_REGISTER, new_integration_time, 2);
            if (ret != 0) {
                return ret;
            }
        ...
        int32_t ret = ARX3A0_WRITE_REG(ARX3A0_GLOBAL_GAIN_REGISTER,
                                       (digital_gain << 7) | (coarse_gain << 4) | fine_gain,
                                       2);
        if (ret != 0) {
            return ret;
        }
    ...
    if (current_integration_time != max_integration_time) {
        resulting_gain =
            (uint32_t) (((float) resulting_gain * current_integration_time) / max_integration_time +
                        0.5f);
    }
