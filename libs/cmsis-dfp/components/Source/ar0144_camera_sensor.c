...
static ARM_DRIVER_GPIO *GPIO_Driver_CAM_RST =
    &ARM_Driver_GPIO_(RTE_AR0144_CAMERA_SENSOR_RESET_GPIO_PORT);
...
static ARM_DRIVER_GPIO *GPIO_Driver_CAM_PWR =
    &ARM_Driver_GPIO_(RTE_AR0144_CAMERA_SENSOR_POWER_GPIO_PORT);
...
/**
  ...
  */
static int32_t AR0144_Bulk_Write_Reg(const AR0144_REG ar0144_reg[], uint32_t total_num,
                                     uint32_t reg_size)
{
    uint32_t i   = 0;
    int32_t  ret = 0;
    ...
    for (i = 0; i < total_num; i++) {
        ...
        ret = AR0144_WRITE_REG(ar0144_reg[i].reg_addr, ar0144_reg[i].reg_value, reg_size);
...
static int32_t AR0144_Camera_Hard_Reseten(void)
{
    ...
    ret = GPIO_Driver_CAM_RST->SetDirection(RTE_AR0144_CAMERA_SENSOR_RESET_PIN_NO,
    ...
    ret = GPIO_Driver_CAM_PWR->SetDirection(RTE_AR0144_CAMERA_SENSOR_POWER_PIN_NO,
                                            GPIO_PIN_DIRECTION_OUTPUT);
    if (ret != ARM_DRIVER_OK) {
        return ret;
    }

    ret = GPIO_Driver_CAM_RST->SetValue(RTE_AR0144_CAMERA_SENSOR_RESET_PIN_NO,
    ...
    ret = GPIO_Driver_CAM_PWR->SetValue(RTE_AR0144_CAMERA_SENSOR_POWER_PIN_NO,
...
/**
  ...
  \note This has been adapted to AR0144 from ARX3A0
        In AR0144 the digital gain and analog gain are in separate registers instead of one as in
        ARX3A0
...
static int32_t AR0144_Camera_Gain(const uint32_t gain)
{
    ...
    if (max_integration_time == 0) {
         ...
         */
        int32_t ret =
            AR0144_READ_REG(AR0144_COARSE_INTEGRATION_TIME_REGISTER, &current_integration_time, 2);
        if (ret != 0) {
            return ret;
        }
    ...
    if (gain != 0) {
        ...
        if (new_integration_time != current_integration_time) {
            int32_t ret =
                AR0144_WRITE_REG(AR0144_COARSE_INTEGRATION_TIME_REGISTER, new_integration_time, 2);
            if (ret != 0) {
                return ret;
            }
...
int32_t AR0144_Init(void)
{
    ...
    total_num =
        (sizeof(cfg_1280x800_12bit_60fps_2Lane_Pxlclk75MHz_Extclk20MHz) / sizeof(AR0144_REG));
...
#elif (RTE_AR0144_CAMERA_SENSOR_FRAME_HEIGHT == 400) &&                                            \
    (RTE_AR0144_CAMERA_SENSOR_FRAME_WIDTH == 640)
#elif (RTE_AR0144_CAMERA_SENSOR_FRAME_HEIGHT == 200) &&                                            \
    (RTE_AR0144_CAMERA_SENSOR_FRAME_WIDTH == 320)
#elif (RTE_AR0144_CAMERA_SENSOR_FRAME_HEIGHT == 100) &&                                            \
    (RTE_AR0144_CAMERA_SENSOR_FRAME_WIDTH == 160)
#else
