...
dc1394error_t dc1394_bayer_NearestNeighbor(const uint8_t *restrict bayer, uint8_t *restrict rgb,
...
dc1394error_t dc1394_bayer_Bilinear(const uint8_t *restrict bayer, uint8_t *restrict rgb, int sx,
                                    int sy, int tile)
{
    const int bayerStep = sx;
    const int rgbStep   = 3 * sx;
    ...
    for (; height--; bayer += bayerStep, rgb += rgbStep) {
        ...
        if (blue > 0) {
            for (; bayer <= bayerEnd - 2; bayer += 2, rgb += 6) {
                t0 = (bayer[0] + bayer[2] + bayer[bayerStep * 2] + bayer[bayerStep * 2 + 2] + 2) >>
                     2;
        ...
        if (bayer < bayerEnd) {
            ...
            t1 = (bayer[1] + bayer[bayerStep] + bayer[bayerStep + 2] + bayer[bayerStep * 2 + 1] +
                  2) >>
                 2;
            rgb[-blue] = (uint8_t) t0;
            rgb[0]     = (uint8_t) t1;
...
dc1394error_t dc1394_bayer_HQLinear(const uint8_t *restrict bayer, uint8_t *restrict rgb, int sx,
                                    int sy, int tile)
{
    const int bayerStep = sx;
    const int rgbStep   = 3 * sx;
    ...
    /* We begin with a (+1 line,+1 column) offset with respect to bilinear decoding, so
     * start_with_green is the same, but blue is opposite */
    blue    = -blue;
    ...
    for (; height--; bayer += bayerStep, rgb += rgbStep) {
        ...
        if (start_with_green) {
            ...
            t0     = rgb[0] * 5 + ((bayer[bayerStep + 2] + bayer[bayerStep3 + 2]) << 2) - bayer[2] -
                 bayer[bayerStep + 1] - bayer[bayerStep + 3] - bayer[bayerStep3 + 1] -
                 bayer[bayerStep3 + 3] - bayer[bayerStep4 + 2] +
            ...
            t1 = rgb[0] * 5 + ((bayer[bayerStep2 + 1] + bayer[bayerStep2 + 3]) << 2) -
                 ...
                 bayer[bayerStep3 + 1] - bayer[bayerStep3 + 3] - bayer[bayerStep2 + 4] +
        ...
        if (blue > 0) {
            for (; bayer <= bayerEnd - 2; bayer += 2, rgb += 6) {
                ...
                /* R at B */
                t0     = ((bayer[bayerStep + 1] + bayer[bayerStep + 3] + bayer[bayerStep3 + 1] +
                       bayer[bayerStep3 + 3])
                      << 1) -
                ...
                /* G at B */
                t1 =
                    ((bayer[bayerStep + 2] + bayer[bayerStep2 + 1] + bayer[bayerStep2 + 3] +
                      bayer[bayerStep3 + 2])
                     << 1) -
                ...
                t0 = rgb[3] * 5 + ((bayer[bayerStep + 3] + bayer[bayerStep3 + 3]) << 2) - bayer[3] -
                     bayer[bayerStep + 2] - bayer[bayerStep + 4] - bayer[bayerStep3 + 2] -
                     bayer[bayerStep3 + 4] - bayer[bayerStep4 + 3] +
                     ((bayer[bayerStep2 + 1] + bayer[bayerStep2 + 5] + 1) >> 1);
                t1 = rgb[3] * 5 + ((bayer[bayerStep2 + 2] + bayer[bayerStep2 + 4]) << 2) -
                     bayer[bayerStep2 + 1] - bayer[bayerStep + 2] - bayer[bayerStep + 4] -
                     bayer[bayerStep3 + 2] - bayer[bayerStep3 + 4] - bayer[bayerStep2 + 5] +
...
dc1394error_t dc1394_bayer_EdgeSense(const uint8_t *restrict bayer, uint8_t *restrict rgb, int sx,
                                     int sy, int tile)
{
...
dc1394error_t dc1394_bayer_Downsample(const uint8_t *restrict bayer, uint8_t *restrict rgb, int sx,
                                      int sy, int tile)
{
...
dc1394error_t dc1394_bayer_Simple(const uint8_t *restrict bayer, uint8_t *restrict rgb, int sx,
                                  int sy, int tile)
{
    const int bayerStep = sx;
    const int rgbStep   = 3 * sx;
...
/* insprired by OpenCV's Bayer decoding */
dc1394error_t dc1394_bayer_NearestNeighbor_uint16(const uint16_t *restrict bayer,
                                                  uint16_t *restrict rgb, int sx, int sy, int tile,
                                                  int bits)
{
    const int bayerStep = sx;
    const int rgbStep   = 3 * sx;
...
/* OpenCV's Bayer decoding */
dc1394error_t dc1394_bayer_Bilinear_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb,
...
/* High-Quality Linear Interpolation For Demosaicing Of
   Bayer-Patterned Color Images, by Henrique S. Malvar, Li-wei He, and
   Ross Cutler, in ICASSP'04 */
dc1394error_t dc1394_bayer_HQLinear_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb,
                                           int sx, int sy, int tile, int bits)
{
    const int bayerStep = sx;
    const int rgbStep   = 3 * sx;
    ...
    for (; height--; bayer += bayerStep, rgb += rgbStep) {
        ...
        if (blue > 0) {
            ...
            for (; bayer <= bayerEnd - 2; bayer += 2, rgb += 6) {
                ...
                rgb[-1] = bayer[bayerStep2 + 2];
                /* B at R */
                t0      = ((bayer[bayerStep + 1] + bayer[bayerStep + 3] + bayer[bayerStep * 3 + 1] +
                       bayer[bayerStep3 + 3])
                      << 1) -
                     (((bayer[2] + bayer[bayerStep2] + bayer[bayerStep2 + 4] +
                        bayer[bayerStep4 + 2]) *
...
/* coriander's Bayer decoding */
dc1394error_t dc1394_bayer_EdgeSense_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb,
                                            int sx, int sy, int tile, int bits)
{
    /* Removed due to patent concerns */
    return DC1394_FUNCTION_NOT_SUPPORTED;
...
/* coriander's Bayer decoding */
dc1394error_t dc1394_bayer_Downsample_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb,
                                             int sx, int sy, int tile, int bits)
{
...
/* coriander's Bayer decoding */
dc1394error_t dc1394_bayer_Simple_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb,
                                         int sx, int sy, int tile, int bits)
{
    uint16_t    *outR, *outG, *outB;
    register int i, j;
...
static const signed char
    bayervng_terms[] = {-2, -2, 0, -1, 0, 0x01, -2, -2, 0, 0, 1, 0x01, -2, -1, -1, 0, 0, 0x01,
...
dc1394error_t dc1394_bayer_VNG(const uint8_t *restrict bayer, uint8_t *restrict dst, int sx, int sy,
...
dc1394error_t dc1394_bayer_VNG_uint16(const uint16_t *restrict bayer, uint16_t *restrict dst,
...
dc1394error_t dc1394_bayer_AHD(const uint8_t *restrict bayer, uint8_t *restrict dst, int sx, int sy,
                               dc1394color_filter_t pattern)
{
        ...
        for (left = 0; left < width; left += TS - 6) {
            ...
            for (row = top < 2 ? 2 : top; row < top + TS && row < height - 2; row++) {
                ...
                for (fc = FC(row, col); col < left + TS && col < width - 2; col += 2) {
                    ...
                    val =
                        ((pix[-1][1] + pix[0][fc] + pix[1][1]) * 2 - pix[-2][fc] - pix[2][fc]) >> 2;
                ...
                for (row = top + 1; row < top + TS - 1 && row < height - 1; row++) {
                    for (col = left + 1; col < left + TS - 1 && col < width - 1; col++) {
                        ...
                        if ((c = 2 - FC(row, col)) == 1) {
                            c   = FC(row + 1, col);
                            val = pix[0][1] +
                                  ((pix[-1][2 - c] + pix[1][2 - c] - rix[-1][1] - rix[1][1]) >> 1);
                            rix[0][2 - c] = CLIPOUT(val); /* [SA] */
                            val =
                                pix[0][1] +
                                ((pix[-width][c] + pix[width][c] - rix[-TS][1] - rix[TS][1]) >> 1);
                        } else {
                            val = rix[0][1] +
                                  ((pix[-width - 1][c] + pix[-width + 1][c] + pix[+width - 1][c] +
                                    pix[+width + 1][c] - rix[-TS - 1][1] - rix[-TS + 1][1] -
                ...
                for (col = left + 3; col < left + TS - 3 && col < width - 3; col++) {
                    ...
                    if (hm[0] != hm[1]) {
                        FORC3 dst[(row * width + col) * 3 + c] =
                            CLIPOUT(rgb[hm[1] > hm[0]][tr][tc][c]); /* [SA] */
                    } else {
                        FORC3 dst[(row * width + col) * 3 + c] =
...
dc1394error_t dc1394_bayer_AHD_uint16(const uint16_t *restrict bayer, uint16_t *restrict dst,
                                      int sx, int sy, dc1394color_filter_t pattern, int bits)
{
    int i, j, top, left, row, col, tr, tc, fc, c, d, val, hm[2];
    /* the following has the same type as the image */
            ...
            for (row = top + 3; row < top + TS - 3 && row < height - 3; row++) {
                ...
                for (col = left + 3; col < left + TS - 3 && col < width - 3; col++) {
                    ...
                    if (hm[0] != hm[1]) {
                        ...
                        FORC3 dst[(row * width + col) * 3 + c] =
                            CLIPOUT16((rgb[0][tr][tc][c] + rgb[1][tr][tc][c]) >> 1,
                                      bits); /* [SA] */
                    }
                }
            }
...
#if ENABLE_8_BIT_VERSION
dc1394error_t dc1394_bayer_decoding_8bit(const uint8_t *restrict bayer, uint8_t *restrict rgb,
                                         uint32_t sx, uint32_t sy, dc1394color_filter_t tile,
