...
void SERVICES_initialize(services_lib_t *init_params)
{
    ...
    s_pkt_buffer_address_global = LocalToGlobal((void *) s_services_host.packet_buffer_address);
 ...
 * @ingroup services-host-handler
 */
void SERVICES_rx_msg_callback(uint32_t receiver_id, uint32_t channel_number, uint32_t service_data)
{
    UNUSED(receiver_id);
    UNUSED(channel_number);
                               ...
                               uint32_t service_timeout)
{
    ...
    uint32_t timeout =
        service_timeout != DEFAULT_TIMEOUT ? service_timeout : s_services_host.wait_timeout;
