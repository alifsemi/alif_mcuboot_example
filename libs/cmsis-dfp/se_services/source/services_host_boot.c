...
uint32_t SERVICES_boot_process_toc_entry(uint32_t services_handle, const uint8_t *image_id,
                                         uint32_t *error_code)
{
    process_toc_entry_svc_t *p_svc =
        (process_toc_entry_svc_t *) SERVICES_prepare_packet_buffer(sizeof(process_toc_entry_svc_t));

    strncpy((char *) p_svc->send_entry_id, (const char *) image_id, IMAGE_NAME_LENGTH);

    uint32_t ret =
        SERVICES_send_request(services_handle, SERVICE_BOOT_PROCESS_TOC_ENTRY, DEFAULT_TIMEOUT);
...
uint32_t SERVICES_boot_cpu(uint32_t services_handle, uint32_t cpu_id, uint32_t address,
                           uint32_t *error_code)
{
    boot_cpu_svc_t *p_svc =
        (boot_cpu_svc_t *) SERVICES_prepare_packet_buffer(sizeof(boot_cpu_svc_t)); /* Srvc bucket */

    p_svc->send_cpu_id  = cpu_id;
    p_svc->send_address = address;

    uint32_t ret        = SERVICES_send_request(services_handle, SERVICE_BOOT_CPU, DEFAULT_TIMEOUT);
    *error_code         = p_svc->resp_error_code;
...
{
    boot_cpu_svc_t *p_svc =
        (boot_cpu_svc_t *) SERVICES_prepare_packet_buffer(sizeof(boot_cpu_svc_t));

    p_svc->send_cpu_id  = cpu_id;
    p_svc->send_address = address;

    uint32_t ret = SERVICES_send_request(services_handle, SERVICE_BOOT_SET_VTOR, DEFAULT_TIMEOUT);
    *error_code  = p_svc->resp_error_code;
    return ret;
}
...
{
    control_cpu_svc_t *p_svc =
        (control_cpu_svc_t *) SERVICES_prepare_packet_buffer(sizeof(control_cpu_svc_t));

    p_svc->send_cpu_id = cpu_id;

    uint32_t ret = SERVICES_send_request(services_handle, SERVICE_BOOT_RESET_CPU, DEFAULT_TIMEOUT);
    *error_code  = p_svc->resp_error_code;
    return ret;
}
 ...
 * If the CPU is running, SERVICES_boot_reset_cpu() must be called before this function to stop the
 * core.
 *
 * @param services_handle
 * @param cpu_id
...
uint32_t SERVICES_boot_release_cpu(uint32_t services_handle, uint32_t cpu_id, uint32_t *error_code)
{
    ...
    uint32_t ret =
        SERVICES_send_request(services_handle, SERVICE_BOOT_RELEASE_CPU, DEFAULT_TIMEOUT);
    *error_code = p_svc->resp_error_code;
    return ret;
}
...
uint32_t SERVICES_boot_reset_soc(uint32_t services_handle)
{
    ...
    return SERVICES_send_request(services_handle, SERVICE_BOOT_RESET_SOC, DEFAULT_TIMEOUT);
}
