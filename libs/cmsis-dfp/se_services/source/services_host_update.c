...
uint32_t SERVICES_update_stoc(uint32_t services_handle, uint32_t image_address, uint32_t image_size,
                              uint32_t *error_code)
{
    update_stoc_svc_t *p_svc =
        (update_stoc_svc_t *) SERVICES_prepare_packet_buffer(sizeof(update_stoc_svc_t));

    p_svc->send_image_address = LocalToGlobal((void *) image_address);
    p_svc->send_image_size    = image_size;
    uint32_t ret = SERVICES_send_request(services_handle, SERVICE_UPDATE_STOC, DEFAULT_TIMEOUT);
