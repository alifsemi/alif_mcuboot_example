...
uint32_t SERVICES_system_get_toc_version(uint32_t services_handle, uint32_t *toc_version,
                                         uint32_t *error_code)
{
    get_toc_version_svc_t *p_svc = (get_toc_version_svc_t *) SERVICES_prepare_packet_buffer(
        sizeof(get_toc_version_svc_t)); /* create packet */
...
uint32_t SERVICES_system_get_toc_number(uint32_t services_handle, uint32_t *toc_number,
                                        uint32_t *error_code)
{
    get_toc_number_svc_t *p_svc =
        (get_toc_number_svc_t *) SERVICES_prepare_packet_buffer(sizeof(get_toc_number_svc_t));

    uint32_t return_code =
        SERVICES_send_request(services_handle, SERVICE_SYSTEM_MGMT_GET_TOC_NUMBER, DEFAULT_TIMEOUT);
                                           ...
                                           SERVICES_toc_data_t *toc_info, uint32_t *error_code)
{
    ...
    return_code = SERVICES_system_get_toc_number(services_handle, &toc_number, error_code);
    ...
    for (uint32_t i = 0; i < toc_number; i++) {
        p_svc = (get_toc_data_t *) SERVICES_prepare_packet_buffer(sizeof(get_toc_data_t));
 ...
 * @ingroup services-host-system
 */
uint32_t SERVICES_system_get_toc_data(uint32_t services_handle, SERVICES_toc_data_t *toc_info,
                                      uint32_t *error_code)
{
...
{
    get_device_part_svc_t *p_svc =
        (get_device_part_svc_t *) SERVICES_prepare_packet_buffer(sizeof(get_device_part_svc_t));
...
uint32_t SERVICES_system_set_services_debug(uint32_t services_handle, bool debug_enable,
                                            uint32_t *error_code)
{
    set_services_capabilities_t *p_svc =
        (set_services_capabilities_t *) SERVICES_prepare_packet_buffer(
            sizeof(set_services_capabilities_t));
...
uint32_t SERVICES_system_get_device_data(uint32_t                 services_handle,
                                         SERVICES_version_data_t *device_info, uint32_t *error_code)
{
    get_device_revision_data_t *p_svc =
        (get_device_revision_data_t *) SERVICES_prepare_packet_buffer(
            sizeof(get_device_revision_data_t));
    uint32_t return_code;
    ...
    memcpy((uint8_t *) &device_info->SerialN[0],
           (uint8_t *) p_svc->SerialN,
           sizeof(device_info->SerialN));
    memcpy((uint8_t *) &device_info->ALIF_PN[0],
           (uint8_t *) p_svc->ALIF_PN,
           sizeof(device_info->ALIF_PN));
    memcpy((uint8_t *) &device_info->HBK0[0], (uint8_t *) p_svc->HBK0, sizeof(device_info->HBK0));
    memcpy((uint8_t *) &device_info->DCU[0], (uint8_t *) p_svc->DCU, sizeof(device_info->DCU));
    memcpy((uint8_t *) &device_info->config[0],
           (uint8_t *) p_svc->config,
           sizeof(device_info->config));
    memcpy((uint8_t *) &device_info->HBK1[0], (uint8_t *) p_svc->HBK1, sizeof(device_info->HBK1));
    memcpy((uint8_t *) &device_info->HBK_FW[0],
           (uint8_t *) p_svc->HBK_FW,
           sizeof(device_info->HBK_FW));
    memcpy((uint8_t *) &device_info->MfgData[0],
           (uint8_t *) p_svc->MfgData,
           sizeof(device_info->MfgData));
    memcpy((uint8_t *) &device_info->external_config,
           (uint8_t *) p_svc->external_config,
...
{
    otp_data_t *p_svc = (otp_data_t *) SERVICES_prepare_packet_buffer(sizeof(otp_data_t));
    uint32_t    return_code;

    p_svc->send_offset = otp_offset;
    return_code =
        SERVICES_send_request(services_handle, SERVICE_SYSTEM_MGMT_READ_OTP, DEFAULT_TIMEOUT);
                                   ...
                                   uint32_t otp_value_word, uint32_t *error_code)
{
    ...
    return_code =
        SERVICES_send_request(services_handle, SERVICE_SYSTEM_MGMT_WRITE_OTP, DEFAULT_TIMEOUT);
                                           ...
                                           uint8_t *eui_extension, uint32_t *error_code)
{
    ...
    return_code = SERVICES_system_get_device_data(services_handle, &device_data, error_code);
                                         ...
                                         uint32_t *error_code)
{
    ...
    return_code = SERVICES_system_get_eui_extension(services_handle, false, device_id, error_code);
    if (return_code != SERVICES_REQ_SUCCESS) {
        return return_code;
    }
...
uint32_t SERVICES_system_get_ecc_public_key(uint32_t services_handle, uint8_t *ecc_pubkey_buffer,
                                            uint32_t *error_code)
{
    get_ecc_pubkey_t *p_svc =
        (get_ecc_pubkey_t *) SERVICES_prepare_packet_buffer(sizeof(get_ecc_pubkey_t));
    uint32_t return_code;
    ...
    *error_code = p_svc->resp_error_code;
    if (return_code == SERVICES_REQ_SUCCESS) {
        memcpy(ecc_pubkey_buffer,
               (const void *) p_svc->resp_ecc_pubkey,
               sizeof(p_svc->resp_ecc_pubkey));
    }
