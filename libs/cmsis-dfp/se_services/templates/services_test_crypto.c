...
static void test_mbedtls_aes(uint32_t services_handle)
{
    ...
    // static const uint8_t NONCE[] = { 0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9,
    // 0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF }; ECB the parameter 'iv' is not used

    // Expected ciphertext
    // OFB
    // static const uint8_t CYPHER[] = { 0xDC, 0x7E, 0x84, 0xBF, 0xDA, 0x79, 0x16, 0x4B, 0x7E, 0xCD,
    // 0x84, 0x86, 0x98, 0x5D, 0x38, 0x60 }; CBC static const uint8_t CYPHER[] = { 0xF5, 0x8C, 0x4C,
    // 0x04, 0xD6, 0xE5, 0xF1, 0xBA, 0x77, 0x9E, 0xAB, 0xFB, 0x5F, 0x7B, 0xFB, 0xD6 }; CTR static
    // const uint8_t CYPHER[] = { 0x60, 0x1E, 0xC3, 0x13, 0x77, 0x57, 0x89, 0xA5, 0xB7, 0xA7, 0xF5,
    // 0x04, 0xBB, 0xF3, 0xD2, 0x28 }; ECB static const uint8_t CYPHER[] = { 0xF3, 0xEE, 0xD1, 0xBD,
    // 0xB5, 0xD2, 0xA0, 0x3C, 0x06, 0x4B, 0x5A, 0x7E, 0x3D, 0xB1, 0x81, 0xF8 };
    ...
    SERVICES_cryptocell_mbedtls_aes_init(services_handle, &error_code, (uint32_t) aes_ctx);
...
static void test_mbedtls_cmac(uint32_t services_handle)
{
    ...
    SERVICES_cryptocell_mbedtls_cmac_init_setkey(services_handle,
                                                 &error_code,
                                                 ctx_addr,
                                                 (uint32_t) cmac_key,
                                                 cmac_key_bits);
    ...
    TEST_print(services_handle,
               ...
               error_code);
    SERVICES_cryptocell_mbedtls_cmac_update(services_handle,
                                            &error_code,
                                            ctx_addr,
                                            (uint32_t) block1,
                                            block1_len);
    TEST_print(services_handle,
               ...
               error_code);
    SERVICES_cryptocell_mbedtls_cmac_finish(services_handle,
                                            &error_code,
                                            ctx_addr,
    ...
    SERVICES_cryptocell_mbedtls_cmac(services_handle,
                                     &error_code,
                                     (uint32_t) cmac_key,
                                     cmac_key_bits,
                                     (uint32_t) block1,
...
static void test_mbedtls_ccm(uint32_t services_handle)
{
    ...
    const uint8_t       AD[20]  = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
                                   0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13};
    const uint8_t       MSG[24] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27,
                                   0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f,
                                   0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37};
    const uint8_t       RES[32] = {0xE3, 0xB2, 0x01, 0xA9, 0xF5, 0xB7, 0x1A, 0x7A, 0x9B, 0x1C, 0xEA,
...
#define MBEDTLS_CIPHER_ID_AES 2  // mbedtls_cipher_id_t
    ...
    // Encryption
    SERVICES_cryptocell_mbedtls_ccm_gcm_set_key(services_handle,
                                                &error_code,
                                                ctx_addr,
                                                MBEDTLS_CCM_KEY,
    ...
    TEST_print(services_handle,
               ...
               error_code);
    SERVICES_cryptocell_mbedtls_ccm_gcm_crypt(services_handle,
                                              &error_code,
                                              ctx_addr,
    ...
    SERVICES_cryptocell_mbedtls_ccm_gcm(services_handle,
                                        &error_code,
                                        MBEDTLS_CCM_ENCRYPT_AND_TAG,
                                        (uint32_t) KEY,
                                        8 * sizeof(KEY),
...
static int mbedtls_ccm_get_security_level(uint8_t sizeOfT, uint8_t *pSecurityLevel)
{
    /*
     ...
     System spec requirement CCM*-3: The CCM* shall support only the security levels that include
     encryption (1XX values).
     */
    switch (sizeOfT) {
    case 0:
...
static int mbedtls_ccm_star_nonce_generate(unsigned char *src_addr, uint32_t frame_counter,
                                           uint8_t size_of_t, unsigned char *nonce_buf)
{
    int     rc                 = 0;
    ...
    memmove(nonce_buf + MBEDTLS_AESCCM_STAR_SOURCE_ADDRESS_SIZE_BYTES,
            &frame_counter,
            sizeof(uint32_t));
...
static void test_mbedtls_ccm_star(uint32_t services_handle)
{
    ...
    const uint8_t        MSG[24] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27,
                                    ...
                                    0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37};
    const uint8_t        RES[]   = {
        0x07, 0x11, 0x7e, 0x35, 0x39, 0xb2, 0xed, 0x4c, 0x84, 0x91, 0x66, 0x07, 0x0b, 0x33,
        0xe6, 0x39, 0x0c, 0xf8, 0xda, 0xcc, 0x37, 0x44, 0x00, 0x9d, 0xb1, 0xac, 0x43, 0x81,
        0x81, 0x8e, 0x17, 0x6d, 0x54, 0x63, 0x78, 0x4b, 0xd7, 0xd0, 0xc5, 0x46,
    };
    ...
    SERVICES_cryptocell_mbedtls_ccm_gcm_crypt(services_handle,
                                              ...
                                              sizeof(MSG),
                                              (uint32_t) nonceBuff,
                                              MBEDTLS_AESCCM_STAR_NONCE_SIZE_BYTES,
                                              (uint32_t) AD,
                                              sizeof(AD),
                                              (uint32_t) MSG,
                                              (uint32_t) buf,
                                              (uint32_t) (buf + sizeof(MSG)),
                                              TAG_LEN);
    TEST_print(services_handle,
               "** TEST SERVICES_cryptocell_mbedtls_ccm_gcm_crypt      service_resp=0x%08X\n",
               error_code);
...
void test_mbedtls_gcm(uint32_t services_handle)
{
    ...
    static const uint8_t additional[] = {0x3A, 0xD7, 0x7B, 0xB4, 0x0D, 0x7A, 0x36, 0x60, 0xA8, 0x9E,
    ...
    static const uint8_t pt[]   = {0xD9, 0x31, 0x32, 0x25, 0xF8, 0x84, 0x06, 0xE5, 0xA5, 0x59, 0x09,
    ...
    static const uint8_t ct[]   = {0x42, 0x83, 0x1E, 0xC2, 0x21, 0x77, 0x74, 0x24, 0x4B, 0x72, 0x21,
...
static void test_mbedtls_chachapoly(uint32_t services_handle)
{
    ...
    const uint8_t  chacha_key[32] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                     0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
                                     0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                                     0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
    static uint8_t chacha_IVCounter[12] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4a, 0x00, 0x00, 0x00, 0x00
    };
    uint8_t chacha_dataIn[] = {0x4c, 0x61, 0x64, 0x69, 0x65, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20,
                               ...
                               0x20, 0x69, 0x74, 0x2e};
    // static const uint8_t expectedDataOut[] = { 0x6e, 0x2e, 0x35, 0x9a, 0x25, 0x68, 0xf9, 0x80,
    // 0x41, 0xba, 0x07, 0x28, 0xdd, 0x0d, 0x69, 0x81, 0xe9, 0x7e, 0x7a, 0xec, 0x1d, 0x43, 0x60,
    // 0xc2, 0x0a, 0x27, 0xaf, 0xcc, 0xfd, 0x9f, 0xae, 0x0b, 0xf9, 0x1b, 0x65, 0xc5, 0x52, 0x47,
    // 0x33, 0xab, 0x8f, 0x59, 0x3d, 0xab, 0xcd, 0x62, 0xb3, 0x57, 0x16, 0x39, 0xd6, 0x24, 0xe6,
    // 0x51, 0x52, 0xab, 0x8f, 0x53, 0x0c, 0x35, 0x9f, 0x08, 0x61, 0xd8, 0x07, 0xca, 0x0d, 0xbf,
    // 0x50, 0x0d, 0x6a, 0x61, 0x56, 0xa3, 0x8e, 0x08, 0x8a, 0x22, 0xb6, 0x5e, 0x52, 0xbc, 0x51,
    // 0x4d, 0x16, 0xcc, 0xf8, 0x06, 0x81, 0x8c, 0xe9, 0x1a, 0xb7, 0x79, 0x37, 0x36, 0x5a, 0xf9,
    // 0x0b, 0xbf, 0x74, 0xa3, 0x5b, 0xe6, 0xb4, 0x0b, 0x8e, 0xed, 0xf2, 0x78, 0x5e, 0x42, 0x87,
    // 0x4d };
    uint32_t chacha_counter = 1;
    uint8_t  chacha_dataOut[114] = {0};

    SERVICES_cryptocell_mbedtls_chacha20_crypt(services_handle,
                                               &error_code,
                                               (uint32_t) chacha_key,
                                               (uint32_t) chacha_IVCounter,
                                               chacha_counter,
                                               sizeof(chacha_dataIn),
                                               (uint32_t) chacha_dataIn,
                                               (uint32_t) chacha_dataOut);
    TEST_print(services_handle,
    ...
    static uint8_t chachapoly_key[] = {0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87,
                                       0x88, 0x89, 0x8a, 0x8b, 0x8c, 0x8d, 0x8e, 0x8f,
                                       0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97,
                                       0x98, 0x99, 0x9a, 0x9b, 0x9c, 0x9d, 0x9e, 0x9f};  // pKey
    uint8_t        chachapoly_context[32];
    memcpy(chachapoly_context, chachapoly_key, sizeof(chachapoly_context));

    static const uint8_t chachapoly_dataIn[] = {
    ...
    static const uint8_t chachapoly_addData[] = {
        0x50, 0x51, 0x52, 0x53, 0xc0, 0xc1, 0xc2, 0xc3, 0xc4, 0xc5, 0xc6, 0xc7
    };
    static uint8_t chachapoly_nonce[] = {
        0x07, 0x00, 0x00, 0x00, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47
    };
    uint8_t chachapoly_mac[16] = {0};  // MBEDTLS_POLY_MAC_SIZE_BYTES
    // static const uint8_t pExpectedDataOut[] = { 0xd3, 0x1a, 0x8d, 0x34, 0x64, 0x8e, 0x60, 0xdb,
    // 0x7b, 0x86, 0xaf, 0xbc, 0x53, 0xef, 0x7e, 0xc2, 0xa4, 0xad, 0xed, 0x51, 0x29, 0x6e, 0x08,
    // 0xfe, 0xa9, 0xe2, 0xb5, 0xa7, 0x36, 0xee, 0x62, 0xd6, 0x3d, 0xbe, 0xa4, 0x5e, 0x8c, 0xa9,
    // 0x67, 0x12, 0x82, 0xfa, 0xfb, 0x69, 0xda, 0x92, 0x72, 0x8b, 0x1a, 0x71, 0xde, 0x0a, 0x9e,
    // 0x06, 0x0b, 0x29, 0x05, 0xd6, 0xa5, 0xb6, 0x7e, 0xcd, 0x3b, 0x36, 0x92, 0xdd, 0xbd, 0x7f,
    // 0x2d, 0x77, 0x8b, 0x8c, 0x98, 0x03, 0xae, 0xe3, 0x28, 0x09, 0x1b, 0x58, 0xfa, 0xb3, 0x24,
    // 0xe4, 0xfa, 0xd6, 0x75, 0x94, 0x55, 0x85, 0x80, 0x8b, 0x48, 0x31, 0xd7, 0xbc, 0x3f, 0xf4,
    // 0xde, 0xf0, 0x8e, 0x4b, 0x7a, 0x9d, 0xe5, 0x76, 0xd2, 0x65, 0x86, 0xce, 0xc6, 0x4b, 0x61,
    // 0x16 };

    SERVICES_cryptocell_mbedtls_chachapoly_crypt(services_handle,
                                                 &error_code,
                                                 (uint32_t) chachapoly_context,
                                                 MBEDTLS_CHACHAPOLY_ENCRYPT_AND_TAG,
                                                 sizeof(chachapoly_dataIn),
                                                 (uint32_t) chachapoly_nonce,
                                                 (uint32_t) chachapoly_addData,
                                                 sizeof(chachapoly_addData),
                                                 (uint32_t) chachapoly_mac,
                                                 (uint32_t) chachapoly_dataIn,
                                                 (uint32_t) chacha_dataOut);
    TEST_print(services_handle,
    ...
    uint8_t poly_key[]    = {0X85, 0Xd6, 0Xbe, 0X78, 0X57, 0X55, 0X6d, 0X33, 0X7f, 0X44, 0X52,
                             0Xfe, 0X42, 0Xd5, 0X06, 0Xa8, 0X01, 0X03, 0X80, 0X8a, 0Xfb, 0X0d,
                             0Xb2, 0Xfd, 0X4a, 0Xbf, 0Xf6, 0Xaf, 0X41, 0X49, 0Xf5, 0X1b};
    uint8_t poly_dataIn[] = {0x43, 0x72, 0X79, 0X70, 0X74, 0X6f, 0X67, 0X72, 0X61, 0X70, 0X68, 0X69,
    ...
    // uint8_t EXPECTED_MAC[] = {0xa8, 0x06, 0x1d, 0xc1, 0x30, 0x51, 0x36, 0xc6, 0xc2, 0x2b, 0x8b,
    // 0xaf, 0x0c, 0x01, 0x27, 0xa9};

    SERVICES_cryptocell_mbedtls_poly1305_crypt(services_handle,
                                               &error_code,
                                               (uint32_t) poly_key,
                                               (uint32_t) poly_dataIn,
                                               sizeof(poly_dataIn),
                                               (uint32_t) poly_macBuf);
    TEST_print(services_handle,
...
{
    static const uint8_t PLAIN[] = {0x6B, 0xC1, 0xBE, 0xE2, 0x2E, 0x40, 0x9F, 0x96, 0xE9, 0x3D,
                                    0x7E, 0x11, 0x73, 0x93, 0x17, 0x2A, 0x6B, 0xC1, 0xBE, 0xE2,
                                    0x2E, 0x40, 0x9F, 0x96, 0xE9, 0x3D, 0x7E, 0x11, 0x73, 0x93,
                                    0x17, 0x2A, 0x6B, 0xC1, 0xBE, 0xE2, 0x2E, 0x40, 0x9F, 0x96,
                                    0xE9, 0x3D, 0x7E, 0x11, 0x73, 0x93, 0x17, 0x2A, 0x6B, 0xC1,
                                    0xBE, 0xE2, 0x2E, 0x40, 0x9F, 0x96, 0xE9, 0x3D, 0x7E, 0x11,
                                    0x73, 0x93, 0x17, 0x2A};
    static const uint8_t KEY[]   = {
        0x60,
        0x3D,
...
static void print_performance(uint32_t services_handle, const char *message, uint32_t start,
                              uint32_t end, uint32_t t1, uint32_t t2)
{
    TEST_print(services_handle,
               "%s: %d (%fs)\n",
               message,
