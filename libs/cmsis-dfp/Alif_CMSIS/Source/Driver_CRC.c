...
/*Driver version*/
static const ARM_DRIVER_VERSION DriverVersion        = {ARM_CRC_API_VERSION, ARM_CRC_DRV_VERSION};
...
/**
  ...
  \return      \ref           execution_status
*/
__STATIC_INLINE int32_t CRC_DMA_Start(DMA_PERIPHERAL_CONFIG *dma_periph, ARM_DMA_PARAMS *dma_params)
{
    int32_t         status;
    ARM_DRIVER_DMA *dma_drv = dma_periph->dma_drv;
...
/**
  ...
  \param[in]   CRC       Pointer to crc resources
*/
static int32_t CRC_DMA_Copy(const void *data_in, uint32_t data_len, uint8_t algo_size,
                            CRC_RESOURCES *CRC)
{
...
/**
 ...
 @param[in]    CRC   : Pointer to CRC resources
 @return       ARM_DRIVER_ERROR_PARAMETER  : if CRC device is invalid
               ARM_DRIVER_OK               : if CRC successfully uninitialized or already not
 initialized
 */
static int32_t CRC_PowerControl(ARM_POWER_STATE status, CRC_RESOURCES *CRC)
{
    if (CRC->state.initialized == 0) {
        return ARM_DRIVER_ERROR;
    }
...
/**
 ...
 initialized
 */
static int32_t CRC_Control(uint32_t control, uint32_t arg, CRC_RESOURCES *CRC)
{
...
/**
@fn         int32_t CRC_Compute (const void *data_in, uint32_t len, uint32_t *data_out,
CRC_RESOURCES *CRC)
@brief      1.To calculate the CRC result for 8 bit 16 bit and 32 bit CRC algorithm.
            ...
            3. For 32 bit CRC our hardware will support for aligned data to calculate the CRC
Result.
            ...
            5. In CRC_calculate_Unaligned function load the aligned CRC result from the hardware ,
               unaligned CRC input,length of unaligned input data and the polynomial for the 32 bit
CRC
...
@return     \ref execution_status
*/
static int32_t CRC_Compute(const void *data_in, uint32_t len, uint32_t *data_out,
                           CRC_RESOURCES *CRC)
{
    int32_t  ret = ARM_DRIVER_OK;
    uint8_t  algo_size;
    ...
    case CRC_8_BIT_SIZE:
        ...
        if (CRC->dma_enable && (CRC->transfer.len > CRC_DMA_MIN_TRANSFER_LEN)) {
            ret = CRC_DMA_Copy(CRC->transfer.data_in, CRC->transfer.len, algo_size, CRC);
        } else
#endif
    ...
    case CRC_32_BIT_SIZE:
        ...
        if (CRC->dma_enable && (CRC->transfer.aligned_len > CRC_DMA_MIN_TRANSFER_LEN)) {
            ret = CRC_DMA_Copy(CRC->transfer.data_in, CRC->transfer.aligned_len, algo_size, CRC);
