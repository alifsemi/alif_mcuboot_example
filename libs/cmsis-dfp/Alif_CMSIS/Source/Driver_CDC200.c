...
/*Driver Version*/
static const ARM_DRIVER_VERSION DriverVersion = {ARM_CDC200_API_VERSION, ARM_CDC200_DRV_VERSION};
...
*/
static int32_t CDC200_Init(ARM_CDC200_SignalEvent_t cb_event, DISPLAY_PANEL_DEVICE *display_panel,
                           CDC_RESOURCES *cdc)
{
    ...
    if (((display_panel->hsync_time + display_panel->hbp_time + display_panel->hactive_time +
          display_panel->hfp_time - 1) > 0xFFFFU)) {
        return ARM_DRIVER_ERROR_PARAMETER;
    }

    if (((display_panel->vsync_line + display_panel->vbp_line + display_panel->vactive_line +
          display_panel->vfp_line - 1) > 0xFFFFU)) {
        return ARM_DRIVER_ERROR_PARAMETER;
    }
...
/**
  ...
  \return      \ref execution_status.
  */
static int32_t CDC200_PowerCtrl(ARM_POWER_STATE state, DISPLAY_PANEL_DEVICE *display_panel,
                                CDC_RESOURCES *cdc)
{
#if (RTE_MIPI_DSI)
    int32_t ret = ARM_DRIVER_OK;
    ...
    case ARM_POWER_FULL:
        {
             ...
             */
            htotal = (display_panel->hsync_time + display_panel->hbp_time +
                      display_panel->hfp_time + display_panel->hactive_time);

            vtotal = (display_panel->vsync_line + display_panel->vbp_line +
                      display_panel->vfp_line + display_panel->vactive_line);

            pixclk_div =
                (int) ((float) GetSystemAXIClock() / (htotal * vtotal * RTE_CDC200_DPI_FPS) + 0.5f);
...
/**
 ...
 \param[in]   control CDC200 contol code operation.
                ...
                - \ref CDC200_FRAMEBUF_UPDATE_VSYNC :     Update layer Frame buffer on vertical
 blanking
 ...
 \return      \ref execution_status.
 */
static int32_t CDC200_control(uint32_t control, uint32_t arg, DISPLAY_PANEL_DEVICE *display_panel,
                              CDC_RESOURCES *cdc)
{
    ...
    case CDC200_CONFIGURE_DISPLAY:
        {
            ...
            cdc_info.line_irq_pos =
                (display_panel->vsync_line + display_panel->vbp_line + display_panel->vactive_line);
            ...
            layer_info.win_info.h_start_pos = (display_panel->hsync_time + display_panel->hbp_time);
            layer_info.win_info.h_stop_pos = ((display_panel->hsync_time + display_panel->hbp_time +
                                               display_panel->hactive_time) -
                                              1);
            layer_info.win_info.v_start_pos = (display_panel->vsync_line + display_panel->vbp_line);
            layer_info.win_info.v_stop_pos = ((display_panel->vsync_line + display_panel->vbp_line +
    ...
    case CDC200_FRAMEBUF_UPDATE:
        {
            ...
            cdc_set_layer_fb_addr(cdc->regs,
                                  CDC_LAYER_1,
                                  CDC_SHADOW_RELOAD_IMR,
                                  LocalToGlobal((void *) arg));
            break;
        }
    ...
    case CDC200_CONFIGURE_LAYER_WINDOW:
        {
            ...
            cdc_set_layer_fb_window(cdc->regs,
                                    (CDC_LAYER) cdc200_layer_info->layer_idx,
                                    CDC_SHADOW_RELOAD_IMR,
                                    &win_info);
            break;
        }
    ...
    case CDC200_CONFIGURE_LAYER_BLENDING:
        {
            ...
            cdc_set_layer_blending(cdc->regs,
                                   (CDC_LAYER) cdc200_layer_info->layer_idx,
                                   CDC_SHADOW_RELOAD_IMR,
                                   cdc200_layer_info->const_alpha,
 ...
 */
static int32_t CDC200_GetVerticalPos(DISPLAY_PANEL_DEVICE *display_panel, CDC_RESOURCES *cdc)
{
    return ((int) cdc_get_y_position_status(cdc->regs) - display_panel->vsync_line -
...
cdc_backgnd_color_info_t BGC_INFO = {.red   = RTE_CDC200_BGC_RED,
