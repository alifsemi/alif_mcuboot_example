...
/* Driver Version */
static const ARM_DRIVER_VERSION DriverVersion = {ARM_CAN_API_VERSION, ARM_CAN_DRV_VERSION};
 ...
 */
static uint8_t CANFD_CalculatePrescaler(CANFD_RESOURCES *CANFD, uint32_t bitrate, uint8_t seg1,
                                        uint8_t seg2)
{
                                  ...
                                  ARM_CAN_SignalObjectEvent_t cb_object_event)
{
    ...
    if ((!blocking_mode) && ((cb_unit_event == NULL) || (cb_object_event == NULL))) {
...
static int32_t ARM_CAN_PowerControl(CANFD_RESOURCES *CANFD, ARM_POWER_STATE state)
{
    ...
    case ARM_POWER_LOW:
        ...
        if (CANFD->state.powered == 0x1U) {
            ...
            if ((canfd_stb_tx_active(CANFD->regs)) || (canfd_ptb_tx_active(CANFD->regs))) {
...
static int32_t ARM_CAN_ObjectSetFilter(CANFD_RESOURCES *CANFD, uint32_t obj_idx,
                                       ARM_CAN_FILTER_OPERATION operation, uint32_t id,
                                       uint32_t arg)
{
    ...
    case ARM_CAN_FILTER_ID_EXACT_ADD:
        ...
        for (filter_num = 0x0U; filter_num < CANFD_MAX_ACCEPTANCE_FILTERS; filter_num++) {
            if (canfd_get_acpt_fltr_status(CANFD->regs, filter_num) ==
                ...
                if ((id & ARM_CAN_OBJECT_FILTER_EXT_FRAMES) == ARM_CAN_OBJECT_FILTER_EXT_FRAMES) {
                                   ...
                                   ARM_CAN_MSG_INFO *msg_info, const uint8_t *data, uint8_t size)
{
    ...
    if (CANFD->data_transfer.tx_header.frame_type) {
        CANFD->data_transfer.tx_header.id =
            (ARM_CAN_EXTENDED_ID(msg_info->id) & (~ARM_CAN_ID_IDE_Msk));
    } else {
    ...
    canfd_select_tx_buf(CANFD->regs, CANFD->data_transfer.tx_header.buf_type);
...
static int32_t ARM_CAN_Control(CANFD_RESOURCES *CANFD, uint32_t control, uint32_t arg)
{
    ...
    case ARM_CAN_CONTROL_RETRANSMISSION:
        ...
        canfd_setup_tx_retrans(CANFD->regs, CANFD_BUF_TYPE_SECONDARY, (bool) arg);
        break;
    ...
    case ARM_CAN_SET_RBUF_OVERFLOW_MODE:
        if (arg == ARM_CAN_RBUF_OVERWRITE_OLD_MSG) {
            ...
            canfd_set_rbuf_overflow_mode(CANFD->regs, CANFD_RBUF_OVF_MODE_OVERWRITE_OLD_MSG);
        } else {
            /* Configures to discard new msg*/
            canfd_set_rbuf_overflow_mode(CANFD->regs, CANFD_RBUF_OVF_MODE_DISCARD_NEW_MSG);
        }
        break;
    ...
    case ARM_CAN_SET_RBUF_STORAGE_FORMAT:
        if (arg == ARM_CAN_RBUF_STORAGE_NORMAL_MSG) {
            ...
            canfd_set_rbuf_storage_format(CANFD->regs, CANFD_RBUF_STORE_NORMAL_MSG);
        } else {
            /* Configures to store normal and error msgs */
            canfd_set_rbuf_storage_format(CANFD->regs, CANFD_RBUF_STORE_ALL_MSG);
        }
        break;
    ...
    case ARM_CAN_SET_TRANSMISSION_MODE:
        ...
        if (canfd_stb_empty(CANFD->regs)) {
            if (arg == ARM_CAN_SET_TRANSMISSION_MODE_FIFO) {
                canfd_set_stb_mode(CANFD->regs, CANFD_SECONDARY_BUF_MODE_FIFO);
            } else {
                canfd_set_stb_mode(CANFD->regs, CANFD_SECONDARY_BUF_MODE_PRIORITY);
            }
        } else {
            return ARM_DRIVER_ERROR_BUSY;
        }
    ...
    case ARM_CAN_CONTROL_PRIMARY_TBUF_RETRANSMISSION:
        ...
        canfd_setup_tx_retrans(CANFD->regs, CANFD_BUF_TYPE_PRIMARY, (bool) arg);
        break;
...
void CANFD_IRQHandler(CANFD_RESOURCES *CANFD)
{
    ...
    if (irq_event & CANFD_RBUF_OVERRUN_EVENT) {
        ...
        irq_event = (CANFD_RBUF_OVERRUN_EVENT | CANFD_RBUF_FULL_EVENT |
                     CANFD_RBUF_ALMOST_FULL_EVENT | CANFD_RBUF_AVAILABLE_EVENT);
        ...
        irq_event =
            (CANFD_RBUF_AVAILABLE_EVENT | CANFD_RBUF_FULL_EVENT | CANFD_RBUF_ALMOST_FULL_EVENT);
    } else if (irq_event & CANFD_RBUF_AVAILABLE_EVENT) {
        /* If the Rx msg available then performs below operation */
        CANFD->cb_obj_event(CANFD->objs[ARM_CAN_OBJ_RX - 0x1U].obj_id, ARM_CAN_EVENT_RECEIVE);
        irq_event =
            (CANFD_RBUF_AVAILABLE_EVENT | CANFD_RBUF_FULL_EVENT | CANFD_RBUF_ALMOST_FULL_EVENT);
        ...
        CANFD->cb_obj_event(CANFD->objs[ARM_CAN_OBJ_TX - 0x1U].obj_id, ARM_CAN_EVENT_SEND_COMPLETE);
...
static CANFD_RESOURCES CANFD0_RES = {.regs          = (CANFD_Type *) CANFD0_BASE,
...
static int32_t ARM_CAN0_SetBitrate(ARM_CAN_BITRATE_SELECT select, uint32_t bitrate,
                                   uint32_t bit_segments)
{
...
static int32_t ARM_CAN0_ObjectSetFilter(uint32_t obj_idx, ARM_CAN_FILTER_OPERATION operation,
                                        uint32_t id, uint32_t arg)
{
    return ARM_CAN_ObjectSetFilter(&CANFD0_RES, obj_idx, operation, id, arg);
}

static int32_t ARM_CAN0_ObjectConfigure(uint32_t obj_idx, ARM_CAN_OBJ_CONFIG obj_cfg)
{
...
static int32_t ARM_CAN0_MessageRead(uint32_t obj_idx, ARM_CAN_MSG_INFO *msg_info, uint8_t *data,
                                    uint8_t size)
{
...
static CANFD_RESOURCES CANFD1_RES = {.regs          = (CANFD_Type *) CANFD1_BASE,
...
static int32_t ARM_CAN1_SetBitrate(ARM_CAN_BITRATE_SELECT select, uint32_t bitrate,
                                   uint32_t bit_segments)
{
...
static int32_t ARM_CAN1_ObjectSetFilter(uint32_t obj_idx, ARM_CAN_FILTER_OPERATION operation,
                                        uint32_t id, uint32_t arg)
{
    return ARM_CAN_ObjectSetFilter(&CANFD1_RES, obj_idx, operation, id, arg);
}

static int32_t ARM_CAN1_ObjectConfigure(uint32_t obj_idx, ARM_CAN_OBJ_CONFIG obj_cfg)
{
...
static int32_t ARM_CAN1_MessageRead(uint32_t obj_idx, ARM_CAN_MSG_INFO *msg_info, uint8_t *data,
                                    uint8_t size)
{
