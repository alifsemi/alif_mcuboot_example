...
/*Driver version*/
static const ARM_DRIVER_VERSION DriverVersion              = {ARM_MIPI_CSI2_API_VERSION,
...
static const CSI_CPI_DATA_MODE_SETTINGS cpi_data_mode_settings[] = {
    {CSI_DT_RAW6,
     CSI_IPI_COLOR_COM_TYPE_COLOR16,
     CPI_COLOR_MODE_CONFIG_IPI16_RAW6,
     CPI_DATA_MODE_BIT_8,
     8},
    {CSI_DT_RAW7,
     CSI_IPI_COLOR_COM_TYPE_COLOR16,
...
static int32_t CSI2_Initialize(ARM_MIPI_CSI2_SignalEvent_t cb_event, CSI_RESOURCES *CSI2)
{
     ...
     * pixel clock = (mipi_clk_freq * number_of_lane * 2 "DDR" * 1.2f "percentage" ) /
     * (bits_per_pixel) */
    pixclock =
        (csi_info->frequency * csi_info->n_lanes * 2 * 1.2f) / cpi_data_mode_settings[index].bpp;
    ...
    if (ipi_info->ipi_mode == CSI_IPI_MODE_CAM_TIMIMG) {
        ...
        /* The rising edge of VSYNC comes at least 3 data clock cycles prior to DATA_EN (alias to
         * HSYNC), So we adding 3 for HSA */
        frame_info->hsa_time = CSI2_HSA_MIN + 1;

        /* For the pixel data to be processed correctly, the back porch time must not be less than
         * the time between the packets plus some margin. (IPI_HSA_TIME + IPI_HBP_TIME)*TIPI >
         * Pkt2PktTime + (2*ShortPktBytes/N_LANES + 2)*TPPI-HS/BytesPerHsClk IPI_HBP_TIME =
         * ((((Pkt2PktTime + (2*ShortPktBytes/N_LANES + 2)*TPPI-HS/BytesPerHsClk) / TIPI) -
         * IPI_HSA_TIME) + 1)
         */
        frame_info->hbp_time =
            ((ceil((pkt2pkt_time_ns + (2 * CSI2_SHORT_PKT_BYTES / csi_info->n_lanes + 2) *
        ...
        /* FIFO underflow occurs when the IPI has a higher throughput than the PHY. Configure the
         * Horizontal Sync Delay (HSD) to avoid FIFO underflow. IPI_HSD_TIME > (Pkt2PktTime +
         * (((LongPktBytes + Bytes2Transmit)/N_LANES)*TPPI-HS/BytesPerHsClk))/TIPI
         *                 - (IPI_HSA_TIME + IPI_HBP_TIME + IPI_HACT_TIME)
         * IPI_HSD_TIME = ((Pkt2PktTime + (((LongPktBytes +
         * Bytes2Transmit)/N_LANES)*TPPI-HS/BytesPerHsClk))/TIPI
         *                 - (IPI_HSA_TIME + IPI_HBP_TIME + IPI_HACT_TIME) + 1)
         */
        frame_info->hsd_time =
            (ceil((pkt2pkt_time_ns +
                   (((CSI2_LONG_PKT_BYTES + bytes_to_transmit) / csi_info->n_lanes) * time_PPI_ns /
         ...
         * MEM_req_1 = (((IPI_HSD_TIME + IPI_HSA_TIME + IPI_HBP_TIME)*TIPI -
         * Pkt2PktTime)/TPPI-HS)*N_LANES*BytesPerHsClk
         *
         * MEM_req_2 = (((IPI_HSD_TIME + IPI_HSA_TIME + IPI_HBP_TIME + IPI_HACT_TIME)*TIPI -
         * Pkt2PktTime
         *              - (((LongPktBytes +
         * Bytes2Transmit)/N_LANES)*TPPI-HS))/TPPI-HS)*N_LANES*BytesPerHsClk
         *
         * Min_Memory_Depth = MAX (MEM_req_1, MEM_req_2, 32)*8/ Memory_Width
         */
        mem_req_1 = (ceil(((frame_info->hsd_time + frame_info->hsa_time + frame_info->hbp_time) *
                               time_IPI_ns -
                           pkt2pkt_time_ns) /
                          time_PPI_ns) *
                     csi_info->n_lanes * CSI2_BYTES_PER_HS_CLK);

        mem_req_2 = (ceil(((frame_info->hsd_time + frame_info->hsa_time + frame_info->hbp_time +
                            camera_sensor->width) *
                               time_IPI_ns -
                           pkt2pkt_time_ns -
                           (((CSI2_LONG_PKT_BYTES + bytes_to_transmit) / csi_info->n_lanes) *
                            time_PPI_ns)) /
                          time_PPI_ns) *
                     csi_info->n_lanes * CSI2_BYTES_PER_HS_CLK);
    ...
    if (csi_info->cpi_cfg.override) {
        for (index = 0;
             index < ARRAY_SIZE(cpi_data_mode_settings) &&
             (cpi_data_mode_settings[index].cpi_color_mode != csi_info->cpi_cfg.cpi_color_mode);
             index++) {
        }
...
static int32_t CSI2_ConfigureIPI(CSI_RESOURCES *CSI2)
{
    ...
    csi_set_horizontal_timing(CSI2->regs,
                              CSI2->ipi_info->frame_info->hsa_time,
                              CSI2->ipi_info->frame_info->hbp_time,
                              CSI2->ipi_info->frame_info->hsd_time,
                              hline_time);

    csi_set_vertical_timing(CSI2->regs,
                            CSI2->ipi_info->frame_info->vsa_line,
