...
/**
 * @fn      int32_t ARM_LPTIMER_Initialize(LPTIMER_RESOURCES *LPTIMER_RES, uint8_t channel,
 * ARM_LPTIMER_SignalEvent_t cb_event)
 ...
 */
static int32_t ARM_LPTIMER_Initialize(LPTIMER_RESOURCES *LPTIMER_RES, uint8_t channel,
                                      ARM_LPTIMER_SignalEvent_t cb_event)
{
    if (cb_event == NULL) {
...
/**
 * @fn      int32_t ARM_LPTIMER_PowerControl (LPTIMER_RESOURCES *LPTIMER, uint8_t channel,
 ...
 * @retval  \ref execution_status
 */
static int32_t ARM_LPTIMER_PowerControl(LPTIMER_RESOURCES *LPTIMER_RES, uint8_t channel,
                                        ARM_POWER_STATE state)
{
    if (channel >= LPTIMER_RES->max_channels) {
        return ARM_DRIVER_ERROR_PARAMETER;
...
/**
 * @fn      int32_t ARM_LPTIMER_Control (LPTIMER_RESOURCES *LPTIMER_RES, uint8_t channel, uint32_t
 * control_code, void *arg)
 ...
 * @retval  \ref execution_status
 */
static int32_t ARM_LPTIMER_Control(LPTIMER_RESOURCES *LPTIMER_RES, uint8_t channel,
                                   uint32_t control_code, void *arg)
{
    if (arg == NULL) {
        return ARM_DRIVER_ERROR_PARAMETER;
    }
