...
/* Driver Version */
static const ARM_DRIVER_VERSION DriverVersion = {ARM_OSPI_API_VERSION, ARM_OSPI_DRV_VERSION};
...
*/
static void OSPI_DMACallback(uint32_t event, int8_t peri_num, OSPI_RESOURCES *OSPI)
{
    if (!OSPI->cb_event) {
        return;
    }
...
/**
  \fn          int32_t OSPI_DMA_PowerControl(ARM_POWER_STATE state, DMA_PERIPHERAL_CONFIG
  *dma_periph) \brief       PowerControl DMA for OSPI \param[in]   state  Power state \param[in]
  dma_periph     Pointer to DMA resources \return      \ref execution_status
*/
__STATIC_INLINE int32_t OSPI_DMA_PowerControl(ARM_POWER_STATE        state,
...
static int32_t ARM_OSPI_Initialize(OSPI_RESOURCES *OSPI, ARM_OSPI_SignalEvent_t cb_event)
{
    ...
    if ((OSPI->tx_fifo_threshold > OSPI_TX_FIFO_DEPTH) ||
        (OSPI->tx_fifo_start_level > OSPI_TX_FIFO_DEPTH))
    {
        return ARM_DRIVER_ERROR_PARAMETER;
    }
...
/**
 * @fn      int32_t ARM_OSPI_Transfer(OSPI_RESOURCES *OSPI, const void *data_out, void *data_in,
 * uint32_t num).
 ...
 * @retval  \ref execution_status
 */
static int32_t ARM_OSPI_Transfer(OSPI_RESOURCES *OSPI, const void *data_out, void *data_in,
                                 uint32_t num)
{
#if OSPI_DMA_ENABLE
    int32_t status;
