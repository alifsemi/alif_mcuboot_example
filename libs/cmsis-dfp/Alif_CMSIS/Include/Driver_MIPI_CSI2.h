...
typedef void (*ARM_MIPI_CSI2_SignalEvent_t)(
    uint32_t int_event);  ///< Pointer to \ref ARM_MIPI_CSI2_SignalEvent : Signal MIPI CSI2 Event.
...
typedef struct {
    ...
    (void);  ///< Pointer to \ref MIPI_CSI2_GetVersion : Get driver version.
    ARM_MIPI_CSI2_CAPABILITIES (*GetCapabilities)
    (void);  ///< Pointer to \ref MIPI_CSI2_GetCapabilities : Get MIPI CSI2 driver capabilities.
    int32_t (*Initialize)(
        ARM_MIPI_CSI2_SignalEvent_t cb_event);  ///< Pointer to \ref MIPI_CSI2_Initialize :
                                                ///< Initialize MIPI CSI2 Interface.
    int32_t (*Uninitialize)(void);  ///< Pointer to \ref MIPI_CSI2_Uninitialize : Uninitialize MIPI
                                    ///< CSI2 Interface.
    int32_t (*PowerControl)(ARM_POWER_STATE state);  ///< Pointer to \ref MIPI_CSI2_PowerControl :
                                                     ///< Control CSI2 Interface Power.
    int32_t (*ConfigureHost)(uint32_t int_event);    ///< Pointer to \ref MIPI_CSI2_ConfigureHost :
                                                     ///< Configure CSI2 Host Interface.
    int32_t (*ConfigureIPI)(void);  ///< Pointer to \ref MIPI_CSI2_StartPHY : Configure CSI2 PHY
                                    ///< Interface.
    int32_t (*StartIPI)(void);      ///< Pointer to \ref MIPI_CSI2_StartIPI : Enable CSI2 IPI
                                    ///< Interface.Get driver version.
