...
typedef void (*ARM_UTIMER_SignalEvent_t)(uint8_t event); /**< Initialization UTIMER call back
...
typedef enum _ARM_UTIMER_TRIGGER_TARGET {
    ...
    ARM_UTIMER_TRIGGER_CAPTURE_A,   /**< Trigger target UTIMER Channel to capture counter value in
                                         Drive A >*/
    ARM_UTIMER_TRIGGER_CAPTURE_B,   /**< Trigger target UTIMER Channel to capture counter value in
                                         Drive B >*/
...
typedef enum _ARM_UTIMER_TRIGGER {
    ...
    ARM_UTIMER_SRC1_DRIVE_A_RISING_B_0,  /**< UTIMER Channel inputs: rising edge on driver A, 0 on
                                              driver B >*/
    ARM_UTIMER_SRC1_DRIVE_A_RISING_B_1,  /**< UTIMER Channel inputs: rising edge on driver A, 1 on
                                              driver B >*/
    ARM_UTIMER_SRC1_DRIVE_A_FALLING_B_0, /**< UTIMER Channel inputs: falling edge on driver A, 0 on
                                              driver B >*/
    ARM_UTIMER_SRC1_DRIVE_A_FALLING_B_1, /**< UTIMER Channel inputs: falling edge on driver A, 1 on
                                              driver B >*/
    ARM_UTIMER_SRC1_DRIVE_B_RISING_A_0,  /**< UTIMER Channel inputs: rising edge on driver B, 0 on
                                              driver A >*/
    ARM_UTIMER_SRC1_DRIVE_B_RISING_A_1,  /**< UTIMER Channel inputs: rising edge on driver B, 1 on
                                              driver A >*/
    ARM_UTIMER_SRC1_DRIVE_B_FALLING_A_0, /**< UTIMER Channel inputs: falling edge on driver B, 0 on
                                              driver A >*/
    ARM_UTIMER_SRC1_DRIVE_B_FALLING_A_1, /**< UTIMER Channel inputs: falling edge on driver B, 0 on
                                              driver A >*/
...
\param[in]  state   : Power state
                    ...
                    - \ref ARM_POWER_LOW :  low power mode: retain state, detect and signal wake-up
                                            events
                    - \ref ARM_POWER_FULL : power on: full operation at maximum performance
\param[out] int32_t : execution_status.

\fn         int32_t ARM_UTIMER_ConfigCounter (uint8_t channel, ARM_UTIMER_MODE mode,
\                                             ARM_UTIMER_COUNTER_DIR dir)
\brief      Configure utimer mode and type.
\param[in]  channel     : Utimer have total 16 channel, configure by feeding TIMER_CHANNEL0, 1, 2,
                          3, 4...
...
typedef struct _ARM_DRIVER_UTIMER {
    /* Pointer to \ref ARM_UTIMER_Initialize    : Initialize the UTIMER Interface */
    int32_t  (*Initialize)     (uint8_t channel, ARM_UTIMER_SignalEvent_t cb_event);
    /* Pointer to \ref ARM_UTIMER_PowerControl  : Control UTIMER interface power> */
    int32_t  (*PowerControl)   (uint8_t channel, ARM_POWER_STATE state);
    /* Pointer to \ref ARM_UTIMER_ConfigCounter : Control timer type and mode for UTIMER channel */
    int32_t  (*ConfigCounter)  (uint8_t channel, ARM_UTIMER_MODE mode, ARM_UTIMER_COUNTER_DIR dir);
    /* Pointer to \ref ARM_UTIMER_SetCount      : Set count value for UTIMER channel */
    int32_t  (*SetCount)       (uint8_t channel, ARM_UTIMER_COUNTER counter, uint32_t value);
    /* Pointer to \ref ARM_UTIMER_GetCount      : Get count value for UTIMER channel */
    uint32_t (*GetCount)       (uint8_t channel, ARM_UTIMER_COUNTER counter);
    /* Pointer to \ref ARM_UTIMER_ConfigTrigger : configure the input triggers       */
    int32_t  (*ConfigTrigger)  (uint8_t channel, ARM_UTIMER_TRIGGER_CONFIG *arg);
    /* Pointer to \ref ARM_UTIMER_Start         : Global starts the UTIMER channel   */
    int32_t  (*Start)          (uint8_t channel);
    /* Pointer to \ref ARM_UTIMER_Stop          : Global stops  the UTIMER channel   */
    int32_t  (*Stop)           (uint8_t channel, bool count_clear_option);
    /* Pointer to \ref ARM_UTIMER_Uninitialize  : Uninitialized the UTIMER Interface */
    int32_t  (*Uninitialize)   (uint8_t channel);
} ARM_DRIVER_UTIMER;
