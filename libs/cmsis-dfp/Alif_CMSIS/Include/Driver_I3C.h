...
#define I3C_MASTER_SET_SLAVE_NACK_RETRY_COUNT                                                      \
    (1UL << 2)  ///< Set retry count to communicate to the slave that nacked the comm. Ref: Slave
                ///< Nack retry count Control arguments
#define I3C_SET_SDA_TX_HOLD_TIME                                                                   \
    (1UL << 3)  ///< Set SDA Hold time(in terms of the CORE_CLK periods) for FM, FM+, SDR and DDR
                ///< mode of operations; arg:<1-7>
#define I3C_MASTER_ABORT_MESSAGE_TRANSFER (1UL << 4)  ///< Abort the current Message transfer
#define I3C_MASTER_SETUP_HOT_JOIN_ACCEPTANCE                                                       \
    (1UL << 5)  ///< Accept/Reject the Hot Join request from slaves; arg: 1-Accept, 0-Reject
#define I3C_MASTER_SETUP_MR_ACCEPTANCE                                                             \
    (1UL << 6)  ///< Accept/Reject the Master request from slaves; arg: 1-Accept, 0-Reject
#define I3C_SLAVE_REQUEST_IBI_BUS_MASTERSHIP (1UL << 7)  ///< Request for Bus Mastership
#define I3C_SET_DEVICE_CHARACTERISTICS                                                             \
    (1UL << 8)                      ///< Set Device Characteristics (DCR) value; arg: 0-255
#define I3C_MASTER_INIT (1UL << 9)  ///< Initialises the device as master
#define I3C_MASTER_SETUP_SIR_ACCEPTANCE                                                            \
    (1UL << 10)  ///< Accept/Reject the SIR from slaves; arg: 1-Accept, 0-Reject
...
#define I3C_SLAVE_NACK_RETRY_COUNT(x)                                                              \
    ((x << I3C_SLAVE_NACK_RETRY_COUNT_Pos) & I3C_SLAVE_NACK_RETRY_COUNT_Msk)  ///< x: 0-3
...
#define I3C_BUS_RESET_SCL_LOW_TIMEOUT_COUNT(x)                                                     \
    ((x & I3C_BUS_RESET_SCL_LOW_TIMEOUT_COUNT_Msk)                                                 \
     << I3C_BUS_RESET_SCL_LOW_TIMEOUT_COUNT_Pos)  // Bus reset scl low timeout cnt: in microsecs
...
#define ARM_I3C_EVENT_SLV_DYN_ADDR_ASSGN                                                           \
    (1UL << 2)  ///< Slave Dynamic Address Assigned(only for Slave mode)
...
#define ARM_I3C_EVENT_SLAVE_LIST                                                                   \
...
#define ARM_I3C_LEC_BUF_UNDER_OVERFLOW_ERROR                                                       \
    (6U)                                       ///< Last error code: Buffer Overflow/underflow error
#define ARM_I3C_LEC_TRANSFER_ABORT_ERROR (7U)  ///< Last error code: Transfer abort error
#define ARM_I3C_LEC_I2C_SLAVE_NACK_ERROR                                                           \
    (8U)  ///< Last error code: I2C slave NACK for current write transfer error
...
typedef void (*ARM_I3C_SignalEvent_t)(uint32_t event);  ///< Pointer to \ref I3C_SignalEvent :
                                                        ///< Signal I3C Event.
...
typedef struct _ARM_DRIVER_I3C {
    ...
    (void);  ///< Pointer to \ref GetVersion        : Get I3C driver version.
    ARM_I3C_CAPABILITIES (*GetCapabilities)
    ...
    int32_t (*Uninitialize)(void);  ///< Pointer to \ref Uninitialize      : De-initialize I3C
                                    ///< Interface.
    int32_t (*PowerControl)(ARM_POWER_STATE state);  ///< Pointer to \ref PowerControl      :
                                                     ///< Control I3C Interface Power.
    int32_t (*MasterTransmit)(uint8_t addr, const uint8_t *data,
                              uint16_t len);  ///< Pointer to \ref MasterTransmit    : Start
                                              ///< transmitting data as I3C Master.
    int32_t (*MasterReceive)(uint8_t addr, uint8_t *data,
                             uint16_t len);  ///< Pointer to \ref MasterReceive     : Start
                                             ///< receiving    data as I3C Master.
    int32_t (*SlaveTransmit)(const uint8_t *data,
                             uint16_t       len);  ///< Pointer to \ref MasterTransmit    : Start
                                             ///< transmitting data as I3C Slave.
    int32_t (*SlaveReceive)(uint8_t *data,
                            uint16_t len);  ///< Pointer to \ref SlaveReceive      : Start receiving
                                            ///< data as I3C Slave.
    int32_t (*Control)(uint32_t control,
                       uint32_t arg);  ///< Pointer to \ref Control           : Control I3C Master
                                       ///< and slave Interface.
    int32_t (*MasterSendCommand)(ARM_I3C_CMD *cmd);    ///< Pointer to \ref MasterSendCommand : Send
                                                       ///< I3C CCC
    ...
    int32_t (*GetSlaveList)(uint8_t *addr_list,
                            uint8_t *count);  ///< Pointer to \ref GetSlaveList      : Fetch list of
                                              ///< slaves attached to the bus.
    int32_t (*GetSlaveDynAddr)(const uint8_t static_addr,
                               uint8_t *dynamic_addr);  ///< Pointer to \ref GetSlaveDynAddr   :
                                                        ///< Fetch dynamic address of slave given
                                                        ///< with static address.
    int32_t (*GetSlvsInfo)(void *data, const uint8_t value);  ///< Pointer to \ref GetSlvsInfo :
                                                              ///< Fetches slave's characteristics.
