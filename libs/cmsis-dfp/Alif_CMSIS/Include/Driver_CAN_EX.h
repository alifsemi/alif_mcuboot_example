...
#define ARM_CAN_SET_SPECIFICATION                                                                  \
    (5UL << ARM_CAN_CONTROL_Pos)  ///< Set Spec: ISO/Non-ISO. Default: ISO
#define ARM_CAN_SET_RBUF_OVERFLOW_MODE                                                             \
    (6UL << ARM_CAN_CONTROL_Pos)  ///< Set mode: overwrite old msg or Don't store new msg
#define ARM_CAN_SET_RBUF_STORAGE_FORMAT                                                            \
    (7UL << ARM_CAN_CONTROL_Pos)  ///< Rx buf storage format. Default: Normal msgs
#define ARM_CAN_SET_RBUF_ALMOST_FULL_WARN_LIMIT                                                    \
    (8UL << ARM_CAN_CONTROL_Pos)  ///< Rx buf almost full warning limit arg: 1-15. Default: 15
#define ARM_CAN_SET_TRANSMISSION_MODE                                                              \
    (9UL << ARM_CAN_CONTROL_Pos)  ///< Normal Transmission buffer mode; Fifo/Priority. Not for
                                  ///< primary buffer
#define ARM_CAN_SET_PRIMARY_TBUF (10UL << ARM_CAN_CONTROL_Pos)  ///< Set Primary Tx buffer
#define ARM_CAN_ABORT_PRIMARY_TBUF_MESSAGE_SEND                                                    \
    (11UL << ARM_CAN_CONTROL_Pos)  ///< Abort Primary buffer message transmission
#define ARM_CAN_CONTROL_PRIMARY_TBUF_RETRANSMISSION                                                \
    (12UL << ARM_CAN_CONTROL_Pos)  ///< Enable/disable primary tbuf's automatic retransmission; arg:
                                   ///< 0 = disable, 1 = enable (default state)
#define ARM_CAN_SET_TIMER_COUNTER                                                                  \
    (13UL << ARM_CAN_CONTROL_Pos)  ///< Set the initial value Timer counter; arg: value
#define ARM_CAN_CONTROL_TIMER_COUNTER                                                              \
    (14UL << ARM_CAN_CONTROL_Pos)  ///< Manipulate Timer counter; arg: Start, Stop, Clear
#define ARM_CAN_ENABLE_TIMESTAMP                                                                   \
    (15UL << ARM_CAN_CONTROL_Pos)  ///< Enable CAN msg timestamp; arg: timestamp position- SOF/EOF
#define ARM_CAN_DISABLE_TIMESTAMP                                                                  \
    (16UL << ARM_CAN_CONTROL_Pos)  ///< Disable CAN msg timestamp; arg: NULL
#define ARM_CAN_GET_TX_TIMESTAMP                                                                   \
    (17UL << ARM_CAN_CONTROL_Pos)  ///< Get CAN Tx msg timestamp; arg: Address of variable to store
                                   ///< it
#define ARM_CAN_GET_RX_TIMESTAMP                                                                   \
    (18UL << ARM_CAN_CONTROL_Pos)  ///< Get CAN Rx msg timestamp; arg: Address of variable to store
                                   ///< it
