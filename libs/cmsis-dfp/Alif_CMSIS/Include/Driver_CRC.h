...
#define ARM_CRC_CONTROL_MASK                                                                       \
    (0x1F << ARM_CRC_CONTROL_POS) /* To select the Reflect, Invert, Bit, Byte, Custom polynomial   \
                                     bit of CRC */
...
#define ARM_CRC_ALGORITHM_SEL_8_BIT_CCITT                                                          \
    (0x01UL)                                  /* To select CRC_8_CCITT algorithm give value as 0 */
...
/**
  ...
  @fn         int32_t CRC_Compute (void *data_in, uint32_t len, uint32_t *data_out,
                                   CRC_resources_t *crc)
...
typedef void (*ARM_CRC_SignalEvent_t)(uint32_t event); /*Pointer to \ref CRC_SignalEvent : Signal
                                                          CRC Event*/
...
typedef struct ARM_Driver_CRC {
    /* pointer is pointing to CRC_GetVersion : used to get the driver version           */
    ARM_DRIVER_VERSION    (*GetVersion)      (void);
    /* pointer is pointing to CRC_Capabilities : used to get the driver capabilities    */
    ARM_CRC_CAPABILITIES  (*GetCapabilities) (void);
    /* Initialize the CRC Interface                                                     */
    int32_t               (*Initialize)      (ARM_CRC_SignalEvent_t cb_event);
    /* Pointer to CRC_Uninitialize : De-initialize CRC Interface                        */
    int32_t               (*Uninitialize)    (void);
    /* Pointer to CRC_PowerControl : Control CRC Interface Power                        */
    int32_t               (*PowerControl)    (ARM_POWER_STATE state);
    /* Pointer to CRC_Control : Control CRC Interface                                   */
    int32_t               (*Control)         (uint32_t control, uint32_t arg);
    /* Pointer to CRC_Seed : used to give the seed value                                */
    int32_t               (*Seed)            (uint32_t seed_value);
    /* Pointer to CRC_PolyCustom : used to give the poly custom value                   */
    int32_t               (*PolyCustom)      (uint32_t polynomial);
    /* Pointer to CRC_Compute : used to give the input data and output data             */
    int32_t               (*Compute)         (const void *data_in, uint32_t len, uint32_t *data_out);
