...
typedef struct _ARM_DRIVER_WDT {
    ...
    (void);  ///< Pointer to \ref WDT_GetVersion : Get driver version.
    ARM_WDT_CAPABILITIES (*GetCapabilities)
    (void);  ///< Pointer to \ref WDT_GetCapabilities : Get driver capabilities.
    int32_t (*Initialize)(uint32_t timeout);  ///< Pointer to \ref WDT_Initialize : Initialize
                                              ///< Watchdog Interface.
    int32_t (*Uninitialize)(void);  ///< Pointer to \ref WDT_Uninitialize : De-initialize Watchdog
                                    ///< Interface.
    int32_t (*PowerControl)(ARM_POWER_STATE state);  ///< Pointer to \ref WDT_PowerControl : Control
                                                     ///< Watchdog Interface Power.
    ...
    int32_t (*Control)(uint32_t control, uint32_t arg);  ///< Pointer to \ref WDT_Control : Control
                                                         ///< Watchdog Interface.
    int32_t (*GetRemainingTime)(uint32_t *val);  ///< Pointer to \ref WDT_GetRemainingTime : Get
                                                 ///< Watchdog remaining time before reset.
