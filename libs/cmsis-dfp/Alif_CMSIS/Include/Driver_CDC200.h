...
typedef enum _CDC200_BLEND_FACTOR {
    ...
    CDC200_BLEND_PIXEL_ALPHA_X_CONST_ALPHA  ///< layer blending factor selected to pixel alpha *
                                            ///< constant alpha
} CDC200_BLEND_FACTOR;
...
/**
  ...
  ARM_CDC200_LAYER_INFO
                ...
                - CDC200_CONFIGURE_LAYER_BLENDING :  Pointer to layer info \ref
  ARM_CDC200_LAYER_INFO \return      \ref execution_status.
...
*/
typedef void (*ARM_CDC200_SignalEvent_t)(
    uint32_t int_event);  ///< Pointer to \ref ARM_CDC200_SignalEvent : Signal CDC200 Event.
...
typedef struct _ARM_DRIVER_CDC200 {
    ...
    (void);  ///< Pointer to \ref ARM_CDC200_GetVersion : Get driver version.
    ARM_CDC200_CAPABILITIES (*GetCapabilities)
    (void);  ///< Pointer to \ref ARM_CDC200_GetCapabilities : Get CDC200 driver capabilities.
    int32_t (*Initialize)(ARM_CDC200_SignalEvent_t cb_event);  ///< Pointer to \ref
                                                               ///< ARM_CDC200_Initialize :
                                                               ///< Initialize CDC200 Interface.
    int32_t (*Uninitialize)(void);  ///< Pointer to \ref ARM_CDC200_Uninitialize : Uninitialize
                                    ///< CDC200 Interface.
    int32_t (*PowerControl)(ARM_POWER_STATE state);  ///< Pointer to \ref ARM_CDC200_PowerControl :
                                                     ///< Control CDC200 Interface Power.
    int32_t (*Control)(uint32_t control, uint32_t arg);  ///< Pointer to \ref ARM_CDC200_Control:
                                                         ///< Control CDC200 Interface.
    int32_t (*GetVerticalPosition)(void);  ///< Pointer to \ref ARM_CDC200_GetVerticalPosition: Get
                                           ///< current vertical position count.
    int32_t (*Start)(void);  ///< Pointer to \ref ARM_CDC200_StartDisplay : Configure CDC200 to
                             ///< start Displaying.
    int32_t (*Stop)(void);   ///< Pointer to \ref ARM_CDC200_StopDisplay : Configure CDC200 to stop
                             ///< Displaying.
} ARM_DRIVER_CDC200;
