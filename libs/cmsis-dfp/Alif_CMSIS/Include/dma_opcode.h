...
#define OP_DMAADDH(ar)                                                                             \
    (0x54 | (ar << 1)) /*!< Adds an immediate 16bit value to SARn or DARn */
#define OP_DMAADNH(ar)                                                                             \
    (0x5C | (ar << 1)) /*!< Adds an immediate 16bit negative value to SARn or DARn */
#define OP_DMAEND                                                                                  \
    0x00 /*!< End signal as the DMAC sequence is complete                                */
#define OP_DMAFLUSHP                                                                               \
    0x35 /*!< Flush the peripheral contents and sends message to resend its level status */
#define OP_DMAGO(ns)                                                                               \
    (0xA0 | (ns << 1)) /*!< Execute thread in secure/non-secure mode */
#define OP_DMAKILL                                                                                 \
    0x01 /*!< Terminate Execution of a thread                                            */
#define OP_DMALD                                                                                   \
    0x04 /*!< Performs DMA Load operation                                                */
#define OP_DMALDS                                                                                  \
    0x05 /*!< Performs DMA Single Load operation                                         */
#define OP_DMALDB                                                                                  \
    0x07 /*!< Performs DMA Burst Load operation                                          */
#define OP_DMALDP(bs)                                                                              \
    (0x25 | (bs << 1)) /*!< Performs DMA Load & Notify Peripheral Single/Burst operation */
#define OP_DMALP(lc)                                                                               \
    (0x20 | (lc << 1)) /*!< Loop instruct DMAC to load 8bit val to LC0/LC1 reg */
#define OP_DMALPEND(nf, lc)                                                                        \
    (0x28 | (nf << 4) | (lc << 2)) /*!< Loop End, nf=0, lc=1 if DMALPFE started loop */
#define OP_DMALPENDS(lc)                                                                           \
    (0x39 | (lc << 2)) /*!< Loop End Single */
#define OP_DMALPENDB(lc)                                                                           \
    (0x3B | (lc << 2)) /*!< Loop End Burst */
#define OP_DMAMOV                                                                                  \
    0xBC /*!< Move 32bit immediate into SAR/DAR/CCR                                      */
#define OP_DMANOP                                                                                  \
    0x18 /*!< For code alignment                                                         */
#define OP_DMARMB                                                                                  \
    0x12 /*!< Read Memory barrier, write-after-read sequence                             */
#define OP_DMASEV                                                                                  \
    0x34 /*!< Send event                                                                 */
#define OP_DMAST                                                                                   \
    0x08 /*!< Performs DMA Store operation                                               */
#define OP_DMASTS                                                                                  \
    0x09 /*!< Performs DMA Single Store operation                                        */
#define OP_DMASTB                                                                                  \
    0x0B /*!< Performs DMA Burst Store operation                                         */
#define OP_DMASTP(bs)                                                                              \
    (0x29 | (bs << 1)) /*!< Performs DMA Store & Notify Peripheral Single/Burst operation */
#define OP_DMASTZ                                                                                  \
    0x0C /*!< Store Zeros                                                                */
#define OP_DMAWFE                                                                                  \
    0x36 /*!< Wait for event                                                             */
#define OP_DMAWFP_P(p)                                                                             \
    (0x30 | (p << 0)) /*!< Wait for peripheral with peripheral bit set */
#define OP_DMAWFP(bs)                                                                              \
    (0x30 | (bs << 1)) /*!< Wait for peripheral in single/burst mode */
#define OP_DMAWMB                                                                                  \
    0x13 /*!< Write memory barrier                                                       */
...
typedef union _dma_ccr_t {
    ...
    struct {
        ...
        uint32_t dst_burst_len : 4; /*!< No of data transfers in a burst when DMAC writes to
                                       destination  */
...
static inline bool dma_construct_add(DMA_REG reg, uint16_t off, dma_opcode_buf *op_buf)
{
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_addneg(DMA_REG reg, int16_t off, dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_3BYTE_LEN) > op_buf->buf_size) {
        return false;
    }
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_flushperiph(uint8_t periph, dma_opcode_buf *op_buf)
{
...
/**
  ...
  \return      void
*/
static inline bool dma_construct_go(DMA_SECURE_STATE ns, uint8_t channel_num, uint32_t imm,
                                    dma_opcode_buf *op_buf)
{
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_loadperiph(DMA_XFER xfer_type, uint8_t periph,
                                            dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_2BYTE_LEN) > op_buf->buf_size) {
        return false;
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_loop(DMA_LC lc, uint8_t iter, dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_2BYTE_LEN) > op_buf->buf_size) {
        return false;
    }
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_loopend(dma_loop_t *lp_args, dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_2BYTE_LEN) > op_buf->buf_size) {
        return false;
    }
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_move(uint32_t imm, DMA_REG reg, dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_6BYTE_LEN) > op_buf->buf_size) {
        return false;
    }
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_send_event(uint8_t event_num, dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_2BYTE_LEN) > op_buf->buf_size) {
        return false;
    }
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_store(DMA_XFER xfer_type, dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_1BYTE_LEN) > op_buf->buf_size) {
        return false;
    }
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_storeperiph(DMA_XFER xfer_type, uint8_t periph,
                                             dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_2BYTE_LEN) > op_buf->buf_size) {
        return false;
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_wfe(bool invalidate, uint8_t event_num, dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_2BYTE_LEN) > op_buf->buf_size) {
        return false;
    }
...
/**
  ...
  \return      bool true if the opcode fits in the allocated space
*/
static inline bool dma_construct_wfp(DMA_XFER xfer_type, uint8_t periph_num, dma_opcode_buf *op_buf)
{
    if ((op_buf->off + DMA_OP_2BYTE_LEN) > op_buf->buf_size) {
        return false;
    }
