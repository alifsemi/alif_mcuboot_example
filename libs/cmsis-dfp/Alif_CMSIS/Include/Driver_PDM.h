...
typedef void (*ARM_PDM_SignalEvent_t)(uint32_t event); /*Pointer to \ref PDM_SignalEvent : Signal
                                                          PDM Event*/
...
typedef struct ARM_DRIVER_PDM {
    ...
    (void); /* pointer is pointing to PDM_GetVersion : used to get the driver version        */
    ARM_PDM_CAPABILITIES (*GetCapabilities)
    (void); /* pointer is pointing to PDM_Capabilities : used to get the driver capabilities */
    int32_t (*Initialize)(ARM_PDM_SignalEvent_t cb_event); /* Pointer pointing to \ref PDM_intialize
                                                            */
    int32_t (*Uninitialize)(void); /* Pointer to PDM_Uninitialize : Un-initialize comparator
                                      Interface              */
    int32_t (*PowerControl)(ARM_POWER_STATE state); /* Pointer to PDM_PowerControl : Control
                                                       Comparator Interface Power              */
    int32_t (*Control)(uint32_t control, uint32_t arg1,
                       uint32_t arg2); /* Pointer to PDM_Control : Control Comparator Interface */
    int32_t (*Config)(PDM_CH_CONFIG *cnfg); /* Pointer to PDM Config: Channel configurations
                                               specific to each channel        */
    int32_t (*Receive)(void    *data,
                       uint32_t num); /* Pointer to Receive : PDM Receive Configuration */
    ARM_PDM_STATUS (*GetStatus)
    (void); /* Pointer to GetStatus: Get the PDM status                                      */
