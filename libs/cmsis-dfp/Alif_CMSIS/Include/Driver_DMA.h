...
#define ARM_DMA_USER_PROVIDED_MCODE                                                                \
    (0x01UL)  ///< Use User provided microcode; arg = microcode address in memory
...
typedef void (*ARM_DMA_SignalEvent_t)(
    uint32_t event, int8_t peri_num);  ///< Pointer to \ref ARM_DMA_SignalEvent : Signal DMA Event.
...
typedef struct _ARM_DRIVER_DMA {
    ...
    (void);  ///< Pointer to \ref ARM_DMA_GetVersion : Get driver version.
    ARM_DMA_CAPABILITIES (*GetCapabilities)
    (void);  ///< Pointer to \ref ARM_DMA_GetCapabilities : Get driver capabilities.
    int32_t (*Initialize)(void);    ///< Pointer to \ref ARM_DMA_Initialize : Initialize DMA
                                    ///< Interface.
    int32_t (*Uninitialize)(void);  ///< Pointer to \ref ARM_DMA_Uninitialize : De-initialize DMA
                                    ///< Interface.
    int32_t (*PowerControl)(ARM_POWER_STATE state);  ///< Pointer to \ref ARM_DMA_PowerControl :
                                                     ///< Control DMA Interface Power.
    int32_t (*Allocate)(DMA_Handle_Type *handle);  ///< Pointer to \ref ARM_DMA_Allocate : Allocate
                                                   ///< a DMA handle
    int32_t (*Control)(DMA_Handle_Type *handle, uint32_t control,
                       uint32_t arg);  ///< Pointer to \ref ARM_DMA_Control : Control DMA operation
    int32_t (*Start)(DMA_Handle_Type *handle,
                     ARM_DMA_PARAMS  *params);  ///< Pointer to \ref ARM_DMA_Start : Start DMA
                                               ///< operation
    int32_t (*Stop)(DMA_Handle_Type *handle);  ///< Pointer to \ref ARM_DMA_Stop : Stop DMA
                                               ///< operation
    int32_t (*GetStatus)(DMA_Handle_Type *handle,
                         uint32_t *count);  ///< Pointer to \ref ARM_DMA_GetStatus : Get DMA status.
    int32_t (*DeAllocate)(DMA_Handle_Type *handle);  ///< Pointer to \ref ARM_DMA_DeAllocate :
                                                     ///< De-Allocate a DMA handle
