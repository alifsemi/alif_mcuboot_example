...
typedef struct _ARM_DRIVER_HWSEM {
    ...
    (void);  ///< Pointer to \ref ARM_HWSEM_GetVersion : Get driver version.
    ARM_HWSEM_CAPABILITIES (*GetCapabilities)
    (void);  ///< Pointer to \ref ARM_HWSEM_GetCapabilities : Get driver capabilities.
    int32_t (*Initialize)(ARM_HWSEM_SignalEvent_t cb_event);  ///< Pointer to \ref
                                                              ///< ARM_HWSEM_Initialize : Initialize
                                                              ///< HW semaphore Device.
    int32_t (*Uninitialize)(void);  ///< Pointer to \ref ARM_HWSEM_Uninitialize : De-initialize HW
                                    ///< Semaphore Device.
    int32_t (*Lock)(void);      ///< Pointer to \ref ARM_HWSEM_Lock : Spin until the semaphore is
                                ///< successfully locked.
    ...
    int32_t (*GetCount)(void);  ///< Pointer to \ref ARM_HWSEM_GetCount : Get the semaphore lock
                                ///< count.
