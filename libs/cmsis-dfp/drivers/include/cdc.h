 ...
 */
#define CDC_Ln_BLEND_CFG_F1_SEL_SHIFT                                                              \
    (8U) /**< CDC layer selection of blending factor f1 position bit*/
...
typedef enum _CDC_BLEND_FACTOR {
    ...
    CDC_BLEND_FACTOR_PIXEL_ALPHA_X_CONST_ALPHA /**< cdc blending factor selected to pixel alpha *
                                                  constant alpha */
} CDC_BLEND_FACTOR;
...
/**
 * @fn      static inline void cdc_set_backgnd_color (CDC_Type *const cdc, const CDC_SHADOW_RELOAD
 sh_rld, const cdc_backgnd_color_info_t *bgc                 )
 * @brief   Set CDC Background color.
 * @param   cdc     Pointer to the cdc register map structure. See {@ref CDC_Type} for details.
 * @param   sh_rld  The shadow register update method. See {@ref CDC_SHADOW_RELOAD} for details.
 * @param   bgc     Pointer to the cdc background color information structure. See (@ref
 cdc_backgnd_color_info_t) for details.
 * @retval  none.
 */
                                         ...
                                         const cdc_backgnd_color_info_t *bgc)
{
    ...
    cdc->CDC_BACKGND_COLOR = ((bgc->red << 16) | (bgc->green << 8) | bgc->blue);
...
void cdc_set_layer_fb_window(CDC_Type *const cdc, const CDC_LAYER layer,
                             const CDC_SHADOW_RELOAD        sh_rld,
                             const cdc_window_info_t *const win_info);
