...
static uint32_t s_sender_frame_count;
 ...
 * @return    None
 */
static void set_irq_enable(MHU_sender_frame_register_t *sender_reg_base, uint32_t mask, bool enable)
{
    if (enable) {
 ...
 * @return    None
 */
static void clear_irq(MHU_sender_frame_register_t *sender_reg_base, uint32_t mask)
{
    ...
    if (status & MHU_CHCOMB) {
        ...
        for (uint32_t channel = 0; channel < 2; channel++) {
            if (chcomb_status & (1 << channel)) {
                ...
                // To know when the transfer has been processed by the Receiver, the sender must
                // wait for all bits in the CH_ST register to read as 0b0.
                if (sender_reg_base->CHANNEL[channel].CH_ST == 0 &&
                    (sender_reg_base->CHANNEL[channel].CH_INT_EN & MHU_CH_CLR)) {
                    // Disable channel interrupt
                    CLEAR_REGISTER_BITS_U32(&sender_reg_base->CHANNEL[channel].CH_INT_EN,
                                            MHU_CH_CLR);
 ...
 * @return
 */
mhu_send_status_t MHU_send_message(uint32_t sender_id, uint32_t channel_number,
 ...
 */
void MHU_sender_initialize(uint32_t sender_frame_base_address_list[], uint32_t sender_frame_count,
