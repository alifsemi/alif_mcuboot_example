...
bool dma_generate_opcode(dma_config_info_t *dma_cfg, uint8_t channel_num)
{
    ...
    ret = dma_construct_move(desc->dst_addr, DMA_REG_DAR, &op_buf);
    if (!ret) {
        return ret;
    }
        ...
        if (desc->direction != DMA_TRANSFER_MEM_TO_MEM) {
            ...
            if (desc->direction == DMA_TRANSFER_MEM_TO_DEV) {
                ...
                if (channel_info->flags & DMA_CHANNEL_FLAG_CRC_MODE) {
                    ...
                    ret = dma_construct_storeperiph(xfer_type, desc->periph_num, &op_buf);
                    if (!ret) {
                        return ret;
                    }
                }
                ...
                ret = dma_construct_loadperiph(xfer_type, desc->periph_num, &op_buf);
                if (!ret) {
                    return ret;
                }
    ...
    if (rem_blen) {
        ...
        if (desc->direction != DMA_TRANSFER_MEM_TO_MEM) {
            if (!(channel_info->flags & DMA_CHANNEL_FLAG_CRC_MODE)) {
                ...
                ret = dma_construct_wfp(DMA_XFER_BURST, desc->periph_num, &op_buf);
                if (!ret) {
                    return ret;
                }
            }
            ...
            if (desc->direction == DMA_TRANSFER_MEM_TO_DEV) {
                ...
                if (channel_info->flags & DMA_CHANNEL_FLAG_CRC_MODE) {
                    ...
                    ret = dma_construct_storeperiph(DMA_XFER_BURST, desc->periph_num, &op_buf);
                    if (!ret) {
                        return ret;
                    }
                }
                ...
                ret = dma_construct_loadperiph(DMA_XFER_BURST, desc->periph_num, &op_buf);
                if (!ret) {
                    return ret;
                }
