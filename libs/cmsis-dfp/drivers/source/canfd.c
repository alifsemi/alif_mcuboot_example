...
/**
  ...
  \return      none
*/
static void canfd_copy_tx_buf(volatile uint32_t *dest, const uint32_t *src, const uint8_t len)
{
    uint8_t  iter     = 0U;
    uint8_t  rem      = 0U;
...
{
    canfd->CANFD_ACFCTRL   = (filter_config.filter & CANFD_ACFCTRL_ACFADR_Msk);

    /* Select AMASK configuration */
    canfd->CANFD_ACFCTRL  |= CANFD_ACFCTRL_SELMASK;

    /* Enable filter */
    canfd->CANFD_ACF_EN_0 |= ((1U << filter_config.filter) & CANFD_ACF_EN_0_AE_X_MAX_Msk);
    ...
    if (filter_config.frame_type == CANFD_ACPT_FILTER_CFG_EXT_FRAMES) {
        filter_config.ac_mask |= (CANFD_ACF_3_MASK_AIDE | CANFD_ACF_3_MASK_AIDEE);
...
void canfd_get_acpt_fltr_data(CANFD_Type *canfd, canfd_acpt_fltr_t *filter_config)
{
    ...
    filter_config->ac_code  = (canfd->CANFD_ACF_0_3_CODE & CANFD_ACF0_3_AMASK_ACODE_X_Msk);

    canfd->CANFD_ACFCTRL   |= CANFD_ACFCTRL_SELMASK;

    filter_config->ac_mask  = (canfd->CANFD_ACF_0_3_MASK & CANFD_ACF0_3_AMASK_ACODE_X_Msk);
...
CANFD_MSG_ERROR canfd_get_last_error_code(CANFD_Type *canfd)
{
    ...
    error         = ((canfd->CANFD_EALCAP & CANFD_EALCAP_KOER_Msk) >> CANFD_EALCAP_KOER_Pos);
    switch (error) {
                                ...
                                const uint8_t prescaler)
{
    ...
    canfd->CANFD_S_SEG_1 = CANFD_DECREMENT((((bitrate_seg >> CANFD_BIT_PROP_SEG_Pos) & 0xFFU) +
                                            ((bitrate_seg >> CANFD_BIT_PHASE_SEG1_Pos) & 0xFFU)),
                                           2U);
...
void canfd_set_err_warn_limit(CANFD_Type *canfd, const uint8_t ewl)
{
    ...
    if (ewl <= CANFD_MAX_ERROR_WARN_LIMIT) {
        ...
        canfd->CANFD_LIMIT |= ((((ewl / 8U) - 1U) << CANFD_LIMIT_EWL_Pos) & CANFD_LIMIT_EWL_Msk);
    } else {
        /* Sets error warning to Max */
        canfd->CANFD_LIMIT |= ((((CANFD_MAX_ERROR_WARN_LIMIT / 8U) - 1U) << CANFD_LIMIT_EWL_Pos) &
                               CANFD_LIMIT_EWL_Msk);
    }
}
                ...
                const uint8_t size)
{
    ...
    tx_msg->control = (CANFD_MSG_IDE(tx_header.frame_type) | CANFD_MSG_RTR(tx_header.rtr) |
                       CANFD_MSG_FDF(tx_header.edl) | CANFD_MSG_BRS(tx_header.brs) |
