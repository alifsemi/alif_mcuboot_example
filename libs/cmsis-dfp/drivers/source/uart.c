 ...
 * @retval  none
 */
static inline void uart_send_a_char_to_thr(UART_Type *uart, char chr)
{
                                    ...
                                    UART_STOP_BITS stop_bits)
{
    ...
    case UART_DATA_BITS_5:
        lcr |= (UART_LCR_DATA_LENGTH_5);
        break;
    case UART_DATA_BITS_6:
        lcr |= (UART_LCR_DATA_LENGTH_6);
        break;
    case UART_DATA_BITS_7:
        lcr |= (UART_LCR_DATA_LENGTH_7);
        break;
    case UART_DATA_BITS_8:
        lcr |= (UART_LCR_DATA_LENGTH_8);
        break;
    }
...
void uart_irq_handler(UART_Type *uart, UART_TRANSFER *transfer)
{
    ...
    case UART_IIR_RECEIVED_DATA_AVAILABLE: /* received data available. */
        ...
        if (transfer->rx_curr_cnt >= transfer->rx_total_num) {
             ...
             * 4.) scenario: expecting 10 bytes but receiving only 1 byte? needs one more upper
             * layer timeout. 5.) or Mark as separate event for "FIFO_EMPTY", Don't include in
             * RX_Timeout.
             */
