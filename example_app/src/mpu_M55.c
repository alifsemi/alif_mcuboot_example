...
#define MEMATTRIDX_NORMAL_WT_RA              3
    ...
    static const ARM_MPU_Region_t mpu_table[] __STARTUP_RO_DATA_ATTRIBUTE =
    {
        ...
        {   /* MRAM - BL + PRIMARY => RO  */
            ...
            .RLAR = ARM_MPU_RLAR(MRAM_START + BOOT_BOOTLOADER_SIZE + BOOT_SLOT_SIZE - 0x21, MEMATTRIDX_NORMAL_WT_RA)
        },
        {   /* MRAM - PRIMARY TRAILER + SECONDARY => RW  */
            .RBAR = ARM_MPU_RBAR(MRAM_START + BOOT_BOOTLOADER_SIZE + BOOT_SLOT_SIZE - 0x20, ARM_MPU_SH_NON, 0, 1, 0),
            .RLAR = ARM_MPU_RLAR(MRAM_START + BOOT_BOOTLOADER_SIZE + 2*BOOT_SLOT_SIZE - 1, MEMATTRIDX_DEVICE_nGnRE)
        },
        {   /* MRAM - SCRATCH + IMAGE 1 BL + IMAGE 1 PRIMARY => RO  */
            .RBAR = ARM_MPU_RBAR(MRAM_START + BOOT_BOOTLOADER_SIZE + 2*BOOT_SLOT_SIZE, ARM_MPU_SH_NON, 1, 1, 0),
            .RLAR = ARM_MPU_RLAR(MRAM_START + IMAGE_1_START + BOOT_BOOTLOADER_SIZE + BOOT_SLOT_SIZE - 0x21, MEMATTRIDX_DEVICE_nGnRE)
        },
        {   /* MRAM - IMAGE 1 PRIMARY TRAILER + SECONDARY => RW  */
            .RBAR = ARM_MPU_RBAR(MRAM_START + IMAGE_1_START + BOOT_BOOTLOADER_SIZE + BOOT_SLOT_SIZE - 0x20, ARM_MPU_SH_NON, 0, 1, 0),
            .RLAR = ARM_MPU_RLAR(MRAM_START + IMAGE_1_START + BOOT_BOOTLOADER_SIZE + 2*BOOT_SLOT_SIZE - 1, MEMATTRIDX_DEVICE_nGnRE)
        },
#if !HE_UPDATES_SERAM
        {   /* MRAM - the rest => RO  */
        ...
        {   /* MRAM - IMAGE 2 PRIMARY => RO  */
            .RBAR = ARM_MPU_RBAR(MRAM_START + IMAGE_1_START + BOOT_BOOTLOADER_SIZE + 2*BOOT_SLOT_SIZE, ARM_MPU_SH_NON, 1, 1, 0),
            .RLAR = ARM_MPU_RLAR(MRAM_START + IMAGE_1_START + BOOT_BOOTLOADER_SIZE + 2*BOOT_SLOT_SIZE + BOOT_SERAM_SLOT_SIZE - 0x21, MEMATTRIDX_NORMAL_WT_RA)
        },
        {   /* MRAM - IMAGE 2 PRIMARY TRAILER + SECONDARY => RW  */
            .RBAR = ARM_MPU_RBAR(MRAM_START + IMAGE_1_START + BOOT_BOOTLOADER_SIZE + 2*BOOT_SLOT_SIZE + BOOT_SERAM_SLOT_SIZE - 0x20, ARM_MPU_SH_NON, 0, 1, 0),
            .RLAR = ARM_MPU_RLAR(MRAM_START + IMAGE_1_START + BOOT_BOOTLOADER_SIZE + 2*BOOT_SLOT_SIZE + 2*BOOT_SERAM_SLOT_SIZE - 1, MEMATTRIDX_DEVICE_nGnRE)
        },
        {   /* MRAM - the rest => RO  */
